...
async function readOrdinaryMsatAssetState({ mintUrl }) {
  return await withActiveBrowserWalletDatabase(async db => {
    const { desired, receipt, canonical, legacy } =
      await readOrdinaryMsatSnapshot(db, mintUrl);
    return {
      backupReady: desired !== undefined
      ...
      selectableCanonicalProofCount: canonical
        .filter(proof => proof.selectability === 'selectable').length,
...
async function evictOrdinaryMsatProofBodies({ mintUrl }) {
  return await withActiveBrowserWalletDatabase(async db => {
    ...
    const verifyTx = db.transaction(
      ['custodyProofs', 'custodyProofBackupAuthorities'],
      'readonly');
    ...
    const [remainingCandidateProofs, remainingCandidateAuthorities] = await Promise.all([
      Promise.all(targetCandidate.proofs.map(proof =>
        read('custodyProofs', [proof.scopeId, proof.proofId]))),
      Promise.all(targetCandidate.proofs.map(proof =>
        read('custodyProofBackupAuthorities', [proof.scopeId, proof.proofId]))),
    ]);
    return {
      remainingCandidateProofCount:
        remainingCandidateProofs.filter(value => value !== undefined).length,
      remainingCandidateProofAuthorityCount:
        remainingCandidateAuthorities.filter(value => value !== undefined).length,
    };
  });
}
