...
function isFundingDepositComplete(
  state: DepositState | null | undefined,
): boolean {
...
function isFundingDepositPending(
  state: DepositState | null | undefined,
): boolean {
  switch (state) {
    case "requested":
...
function assertNeverPaymentResult(result: never): never {
  throw new Error(`Unhandled local-wallet payment result: ${String(result)}`);
}
...
function customBudgetInputToSubunits(
  ...
  if (baseAsset === "sat")
    return Math.max(0, Math.floor(customBudgetInput * 1000));
  throw new Error(`unsupported base asset: ${baseAsset}`);
}
...
function customBudgetInputMode(
  baseAsset: MarketBaseAsset,
): "decimal" | "numeric" {
...
export function DepositStep({
  ...
  const [selectedTier, setSelectedTier] =
    useState<AmmFundingTierId>("standard");
  ...
  const customBudgetSubunits = customBudgetInputToSubunits(
    customBudgetInput,
    baseAsset,
  );
  ...
  const selectedTierBudget =
    tiers.find((tier) => tier.id === selectedTier)?.budgetSats ??
    customBudgetSubunits;
  const budgetSats =
    selectedTier === "custom" ? customBudgetSubunits : selectedTierBudget;
  const customBudgetPreview = formatFundingBudget(
    customBudgetSubunits,
    fundingUnit,
  );
  ...
  const resolveDepositIdentity = useCallback(() => {
    const settings = useSettingsStore.getState();
    const fundingIdentity = resolveCreatorPubkey({
      nostrSignerMode: settings.nostrSignerMode,
      nsecSecret: settings.nsecSecret,
      nostrProfilePubkey: settings.nostrProfile?.pubkey,
    });
    if (!fundingIdentity) {
      throw new Error("Ecash deposit authentication is unavailable");
    }
    return { fundingIdentity, creatorPubkey: fundingIdentity };
  }, []);

  const submitLocalWalletToken = useCallback(
    async (submission: GuiEcashDepositSubmission) => {
      const { depositId, request, token } = submission;
      return await requestEcashDeposit(
        request.conditionId,
        depositId,
  ...
  const readLocalWalletDepositStatus = useCallback<
    GuiEcashDepositStatusReader["getStatus"]
  >(
    async ({ depositId, request }) =>
      await getDepositStatus(request.conditionId, depositId),
    [],
  );

  const applyLocalWalletPaymentResult = useCallback(
    (result: GuiLocalWalletPaymentResult) => {
      switch (result.status) {
        case "completed":
          setError(null);
          setDepositState("credited");
          setActiveDepositId(null);
          return;
        case "pending":
          setError(null);
          setDepositState(result.remoteState);
        ...
        case "insufficient":
          return;
        default:
          return assertNeverPaymentResult(result);
      }
    },
    [],
  );
  ...
  useEffect(() => {
    if (!activeDepositId || !isFundingDepositPending(depositState))
      return undefined;
    let cancelled = false;
    ...
    const poll = async () => {
      try {
        const result = await observeGuiEcashDeposit(activeDepositId, {
          getStatus: readLocalWalletDepositStatus,
        });
        if (cancelled) return;
        if (result === null) {
          schedulePoll();
        ...
        switch (result.status) {
          case "transport-ambiguous":
            ...
            return;
          case "pending":
            applyLocalWalletPaymentResult(result);
            if (isFundingDepositPending(result.remoteState)) schedulePoll();
            return;
          case "completed":
          case "insufficient":
            applyLocalWalletPaymentResult(result);
    ...
    activeDepositId,
    applyLocalWalletPaymentResult,
    depositState,
    readLocalWalletDepositStatus,
    t,
  ]);
    ...
    <div className="w-full max-w-2xl">
        ...
        {tiers.map((tier) => (
          ...
          >
            ...
            <span className="mt-1 block text-lg font-bold text-slate-100">
              {formatFundingBudget(tier.budgetSats, fundingUnit, {
                wholeUsd: true,
              })}
            </span>
            {tier.warning && (
      ...
      {showWarning && (
        <div className="mb-5 rounded-lg border border-amber-400/30 bg-amber-400/10 p-3 text-sm text-amber-100">
          ...
          <span className="ml-2">
            {t("marketCreation.ammFundingMinimalWarning")}
          </span>
        </div>
      )}
      ...
      {selectedTier === "none" ? (
        ...
        <LocalWalletPayButton
          ...
          fundAmm
          resolveFundingIdentity={resolveDepositIdentity}
          getTokenPaymentStatus={readLocalWalletDepositStatus}
          retryDepositId={activeDepositId}
          onPaymentResult={applyLocalWalletPaymentResult}
          pending={isFundingDepositPending(depositState)}
          failed={
            activeDepositId !== null && !isFundingDepositPending(depositState)
          }
          disabled={
            budgetSats < 1 ||
            isFundingDepositComplete(depositState) ||
            isFundingDepositPending(depositState)
          }
          onTokenPayment={submitLocalWalletToken}
        />
      )}
