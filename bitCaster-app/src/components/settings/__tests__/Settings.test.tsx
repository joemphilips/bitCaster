...
function settingsState(overrides: Partial<SettingsState["nostr"]> = {}): SettingsState {
  return {
    general: {
      baseCurrency: "BTC",
      language: "en",
...
describe("Settings liked-market close notifications opt-in (P22 Link G)", () => {
  ...
  function generalSettings(likedMarketCloseNotifications: boolean): SettingsState {
  ...
  it("reflects the opt-in OFF state on the toggle switch", () => {
    render(<Settings activeCategory="general" settings={generalSettings(false)} />);
  ...
  it("reflects the opt-in ON state on the toggle switch", () => {
    render(<Settings activeCategory="general" settings={generalSettings(true)} />);
    const toggle = screen.getByRole("switch", {
      name: /notify when a liked market closes/i,
    });
  ...
  it("invokes the change handler when toggled — the handler gates the SW permission request", async () => {
    ...
    fireEvent.click(screen.getByRole("switch", { name: /notify when a liked market closes/i }));
