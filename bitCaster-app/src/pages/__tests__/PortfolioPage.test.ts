...
describe("toPortfolioMarketDetailId", () => {
  it("strips the outcome suffix from per-outcome market ids", () => {
    expect(toPortfolioMarketDetailId("abc123-YES", "YES")).toBe("abc123");
    expect(toPortfolioMarketDetailId("abc123-A|B", "A|B")).toBe("abc123");
  ...
  it("leaves condition ids unchanged", () => {
    ...
    expect(toPortfolioMarketDetailId("condition-with-dashes")).toBe("condition-with-dashes");
  });
});
