...
export function migratePartialLockFailureState(
  ...
  for (const [tradeId, raw] of Object.entries(byTradeId)) {
    ...
    const metadata: OutcomeMetadata = {
      ...
      outcomeCollection: typeof legacy.outcomeCollection === 'string' ? legacy.outcomeCollection : '',
    ...
    const recordTradeId =
      typeof legacy.tradeId === 'string' && legacy.tradeId.length > 0
        ? legacy.tradeId
        : tradeId
    migrated[recordTradeId] = {
      ...
      refundLocktime:
        typeof legacy.refundLocktime === 'number' ? legacy.refundLocktime : 0,
      affectedKeysets,
