...
describe("useActiveSwapsStore", () => {
  it("keeps protocol state on the active swap entry", () => {
    ...
    useActiveSwapsStore.getState().setSellerState("trade-1", { adaptorPoint });
  ...
  it("claims one in-flight step at a time and releases it", () => {
    ...
    expect(useActiveSwapsStore.getState().claimStep("trade-1", "settle")).toBe(true);
    expect(useActiveSwapsStore.getState().claimStep("trade-1", "settle")).toBe(false);
