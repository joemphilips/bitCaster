...
export async function recoverGuiDurableTradeSession(
  ...
  return withGuiSwapSessionOwnership(tradeId, async () => {
    ...
    return recoverDurableTradeSessions({
      ...input,
      sessions: guiDurableTradeSessionRepository(tradeId),
      operations: guiDurableProofOperationRepository(tradeId),
      atomicTransition: {
        advance: async ({ session, operation, state }) =>
          advanceGuiDurableTradeAtomically(session, operation, state),
      },
    })
  })
}
...
function guiDurableTradeSessionRepository(
  ...
  return {
    ...
    compareAndSwap: async (requestedTradeId, expectedRevision, next) => {
      ...
      await db.transaction('rw', db.swapSessions, async () => {
        const current = await db.swapSessions.get(tradeId)
        if (
...
function guiDurableProofOperationRepository(
  ...
  return {
    get: async (operationId) => {
      const operation = await findGuiProofOperationByDurableId(operationId)
    ...
    markMintSubmitted: async (operationId) =>
      updateGuiDurableProofOperation(tradeId, operationId, 'mint-submitted'),
    markReconciled: async (operationId) =>
      updateGuiDurableProofOperation(tradeId, operationId, 'reconciled'),
  }
}
...
async function advanceGuiDurableTradeAtomically(
  ...
  await db.transaction('rw', db.proofOperations, db.swapSessions, async () => {
    const sessionRow = await db.swapSessions.get(session.tradeId)
    if (
...
async function findGuiProofOperationByDurableId(
  ...
  const operation = await db.proofOperations
