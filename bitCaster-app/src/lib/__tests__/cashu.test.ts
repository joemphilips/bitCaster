...
describe('decodeToken real v4 fixture', () => {
  ...
  it('decodes a real testnut token by fetching keysets from the token mint', async () => {
    ...
    expect(
      decoded.proofs.reduce((sum, proof) => sum + amountToNumber(proof.amount), 0),
    ).toBe(50)
    expect([...new Set(decoded.proofs.map((proof) => proof.id))]).toEqual([
      TESTNUT_SAT_KEYSET,
    ])
