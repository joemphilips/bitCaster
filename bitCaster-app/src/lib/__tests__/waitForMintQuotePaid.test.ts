...
const mockMintQuotePaidOn =
  vi.fn<
    (
      id: string,
      paid: (q: PartialMintQuoteResponse) => void,
      err: (e: Error) => void,
    ) => Promise<() => void>
  >();
...
const fakeWallet = {
  ...
  on: {
    mintQuotePaid: (
      id: string,
      p: (q: PartialMintQuoteResponse) => void,
      e: (err: Error) => void,
    ) => mockMintQuotePaidOn(id, p, e),
  },
};
...
describe("waitForMintQuotePaid", () => {
  ...
  it("fires EXPIRED at the bolt11 expiry boundary instead of polling forever", async () => {
    ...
    await waitForMintQuotePaid(makeQuote({ expiry: NOW_SEC + 5 }), onResult, {
