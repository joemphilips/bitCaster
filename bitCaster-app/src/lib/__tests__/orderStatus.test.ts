...
import { configureGuiWalletIdProvider } from "@/stores/proof-db";
import {
...
describe("promoteNewFillsToActiveSwaps", () => {
  ...
  it("hydrates a missing pending key from the durable intent before promoting its fill", async () => {
    ...
    await vi.waitFor(() => {
      expect(
        useActiveSwapsStore.getState().byTradeId["trade-durable"],
      ).toBeDefined();
    });
  });
  ...
  it("retains the taker role and exact fill amount for failed-fill recovery", () => {
    ...
    expect(
      useActiveSwapsStore.getState().byTradeId["trade-taker"],
    ).toMatchObject({
      isTaker: true,
...
function pendingTrade(
  overrides: Partial<PendingTradeRecord> = {},
): PendingTradeRecord {
