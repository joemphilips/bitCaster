...
vi.mock("@cashu/cashu-ts", async (importOriginal) => {
  ...
  class MockOutputData {
    ...
    toProof(signature: { C_?: string }): Proof {
      return makeProof(
        new TextDecoder().decode(this.secret) || `restored-${Array.from(this.secret).join("-")}`,
    ...
    Wallet: vi.fn(function MockWallet(_mint: unknown, options?: { unit?: string }) {
      ...
      return {
        ...
        send: vi
          .fn()
          .mockImplementation(
            async (_amount: number, proofs: Proof[], _config?: unknown, outputConfig?: any) => {
        ...
        prepareSwapToReceive: vi.fn().mockImplementation(async (token: { proofs: Proof[] }) => {
          ...
          const amount = token.proofs.reduce((sum, p) => sum + amountToNumber(p.amount), 0);
          return {
            amount,
            fees: 0,
        ...
        prepareConditionalSwap: vi.fn().mockImplementation(
          async (options: {
            inputs: Proof[];
            outputs: Array<{
              label: string;
            ...
            outputDataByLabel: Object.fromEntries(
              options.outputs.map((group) => [
                ...
                group.kind === "p2pk"
                  ...
                  : MockOutputData.createRandomData(group.amount, {
                      id: options.inputs[0]?.id ?? "conditional-keyset",
                    }),
              ]),
            ),
        ...
        completeConditionalSwap: vi
          .fn()
          .mockImplementation(
            async (preview: { outputDataByLabel: Record<string, MockOutputData[]> }) => {
              cashuMockState.mintSwapCalls++;
              if (cashuMockState.conditionalSwapError) {
                throw cashuMockState.conditionalSwapError;
              }
        ...
        completeSwap: vi.fn().mockImplementation(
          async (preview: {
            sendOutputs?: Array<{
            ...
            return {
              send: (preview.sendOutputs ?? []).map((o, index) =>
                o.toProof
                  ...
                  : makeProof(
                      `send-${index}`,
                      Number(o.blindedMessage.amount),
                      o.blindedMessage.id,
                    ),
              ),
...
vi.mock("../../../../cashu-ts/lib/cashu-ts.es.js", () => {
  ...
  class MockCtfOutputData {
    ...
    static createRandomData(amount: MockCtfAmount, keyset: { id: string }): MockCtfOutputData[] {
...
vi.mock("@/stores/proof-db", () => ({
  getProofOperation: vi.fn(
    async (operationId: string) => proofDbMockState.operations.get(operationId) ?? null,
  ),
  ...
  markProofOperationCompleted: vi.fn(
    async (operationId: string, resultProofs: Record<string, Proof[]>) => {
...
describe("validateLocktimeOrdering", () => {
  ...
  it("accepts the minimum gap exactly above Δ", () => {
    expect(validateLocktimeOrdering(buyer + MIN_LOCKTIME_DELTA_SECS + 1, buyer)).toBeNull();
  });
  ...
  it("rejects when the gap is exactly Δ (boundary is strict)", () => {
    expect(validateLocktimeOrdering(buyer + MIN_LOCKTIME_DELTA_SECS, buyer)).toMatch(
      /locktime ordering/i,
    );
  });
...
describe("conditionalKeysetSwap", () => {
  ...
  it("mints every output on the source conditional keyset and persists the operation", async () => {
    ...
    expect(
      proofDbMockState.operations.get("trade-conditional/browser/conditional-keyset-swap"),
    ).toEqual(
  ...
  it("normalizes cashu-ts BigInt proof amounts before persisting operations", async () => {
    ...
    expect(() =>
      JSON.stringify(
        proofDbMockState.operations.get("trade-conditional/browser/bigint-proof-amount")
          ?.resultProofs,
      ),
    ).not.toThrow();
  });
  ...
  it("restores prepared conditional outputs when inputs are already spent", async () => {
    ...
    const prepared = proofDbMockState.operations.get("trade-conditional/browser/restore");
...
describe("sellerLockOutcomeProofs", () => {
  it("locks the requested amount and returns unlocked conditional-keyset change", async () => {
    ...
    await expect(sellerPreparePrelockedSwap(sellerCtx, locked.lockedProofs)).resolves.toEqual(
      expect.objectContaining({
...
describe("buyerClaimSwap", () => {
  ...
  it("fails loudly when the mint rejects same-keyset conditional refresh", async () => {
    ...
    cashuMockState.conditionalSwapError = new Error("Inputs must use the same conditional keyset");
  ...
  it("claims primitive multi-keyset conditional proofs one keyset at a time", async () => {
    ...
    expect(proofDbMockState.operations.get(`${operationId}:0:keyset-B`)).toEqual(
      expect.objectContaining({ kind: "conditional-keyset-swap" }),
    );
    expect(proofDbMockState.operations.get(`${operationId}:1:keyset-C`)).toEqual(
      expect.objectContaining({ kind: "conditional-keyset-swap" }),
    );
  });
});
...
describe("splitProofsForExactSend", () => {
  it("splits an oversized reserved proof into exact send proofs and change", async () => {
    ...
    expect(split.sendProofs).toEqual([expect.objectContaining({ amount: 100 })]);
    expect(split.changeProofs).toEqual([expect.objectContaining({ amount: 36 })]);
    expect(split.spentProofs).toEqual([expect.objectContaining({ secret: "reserved-no-136" })]);
  });
...
describe("sellerPrepareSwap", () => {
  it("surfaces mint errors when regular direct locking fails", async () => {
    ...
    await expect(sellerPrepareSwap(ctx, [proof("regular-proof", 1)])).rejects.toThrow(
...
describe("browser proof operation recovery", () => {
  ...
  it("restores prepared claim outputs when the mint spent inputs before the browser persisted fresh proofs", async () => {
    ...
    await expect(
      sellerClaimSwap(sellerCtx, sellerOut.adaptorPoint, buyerOut.lockedProofsCipher, {
        operationId,
        proofOperationStore,
      }),
