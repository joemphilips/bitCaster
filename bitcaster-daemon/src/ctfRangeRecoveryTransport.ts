...
import { readAllocationBoundedJsonResponse } from '@bitcaster-market/client-sdk/boundedJsonResponse'
import type {
...
const DAEMON_CTF_RANGE_MINT_CHECK_RESPONSE_BYTES_MAX = 384 * 1_024
const DAEMON_CTF_RANGE_MINT_KEYS_RESPONSE_BYTES_MAX = 128 * 1_024
export const DAEMON_CTF_RANGE_MINT_RESTORE_RESPONSE_BYTES_MAX = 384 * 1_024
const UUID_PATTERN = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/
const DIGEST_PATTERN = /^[0-9a-f]{64}$/

export interface DaemonCtfRangeEngineResult {
...
export interface DaemonCtfRangeProofStateClient {
  check(payload: CheckStatePayload): Promise<CheckStateResponse>
}

export interface DaemonCtfRangeInputProofIdentity {
  readonly id: string
  readonly secret: string
}

export async function fetchDaemonCtfRangeEngineResultByOperation(
  client: DaemonCtfRangeEngineResultClient,
  authority: DaemonCtfRangeEngineResultAuthority,
): Promise<DaemonCtfRangeEngineResult | null> {
  ...
  try {
    ...
    throw new Error('daemon CTF range engine result request failed')
  }
  return response === null
    ? null
    : decodeDaemonCtfRangeEngineResult(response, { ...authority, operation })
}

export function decodeDaemonCtfRangeEngineResult(
  value: SettlementCapabilityResultResponse,
  authority: DaemonCtfRangeEngineResultAuthority,
): DaemonCtfRangeEngineResult {
  try {
    ...
    const computedDigest = createHash('sha256').update(envelopeBytes).digest('hex')
    ...
    throw new Error('daemon CTF range engine result is invalid')
  }
}
 ...
 */
export async function checkDaemonCtfRangeInputProofStates(
  mint: DaemonCtfRangeProofStateClient,
  inputs: readonly DaemonCtfRangeInputProofIdentity[],
): Promise<ProofState[]> {
  try {
    ...
    throw new Error('daemon CTF range proof-state response is invalid')
  }
}

export class DaemonCtfRangeMintRecoveryAdapter {
  readonly #operation: DurableCtfRangeOperation
  readonly #mint: DaemonCtfRangeMintClient

  constructor(
    operation: DurableCtfRangeOperation,
    mint?: DaemonCtfRangeMintClient,
    ...
    if (this.#operation.unit !== 'msat') {
      throw new Error('daemon CTF range mint authority is invalid')
    }
  ...
  async classifyUncertainRecovery(input: {
    ...
    try {
      ...
      const [recovery, inputStates, resolveKeyset] = await Promise.all([
        ...
        checkDaemonCtfRangeInputProofStates(this.#mint, this.#operation.inputs),
      ...
      throw new Error('daemon CTF range mint recovery failed')
    }
  }
...
function createBoundedCashuRequest(canonicalMintUrl: string, fetchImpl: typeof fetch): RequestFn {
  return async function boundedCashuRequest<T = unknown>(options: RequestOptions): Promise<T> {
    ...
    try {
      ...
      throw new Error('daemon CTF range mint request failed')
    } finally {
      lifetime.dispose()
    }
  }
...
function cashuResponseByteLimit(canonicalMintUrl: string, options: RequestOptions): number {
  ...
  if (endpoint.pathname === '/v1/restore' && method === 'POST') {
    return DAEMON_CTF_RANGE_MINT_RESTORE_RESPONSE_BYTES_MAX
  }
  if (endpoint.pathname === '/v1/checkstate' && method === 'POST') {
    return DAEMON_CTF_RANGE_MINT_CHECK_RESPONSE_BYTES_MAX
  }
  if (/^\/v1\/keys\/[^/]+$/.test(endpoint.pathname) && method === 'GET') {
    return DAEMON_CTF_RANGE_MINT_KEYS_RESPONSE_BYTES_MAX
  }
...
function decodeCanonicalBase64Envelope(value: string): Uint8Array {
  ...
  if (bytes.byteLength > DURABLE_CTF_RANGE_RESULT_BYTES_MAX || bytes.toString('base64') !== value) {
