...
export class DaemonSwapExecutor {
  ...
  private async sellerOpen(tradeId: string): Promise<void> {
    ...
    try {
      ...
      const selected = selectOutcomeProofsForOutcomeSet(
        await readState(),
        profile.mintUrl,
        asset.conditionId,
        asset.outcomeSetId,
      ...
      const collectionByKeyset = keysetToOutcomeCollection(selected)
  ...
  private async sellerOpenMint(
    ...
    const availableOutcome = selectOutcomeProofsForOutcomeSet(
      await readState(),
      mintUrl,
      split.conditionId,
...
function selectOutcomeProofsForOutcomeSet(
  state: DaemonState | null,
  mintUrl: string,
  conditionId: string,
  outcomeSetId: string,
...
function selectReservedProofsForOutcomeSet(
  state: DaemonState | null,
  mintUrl: string,
  reservedBy: string,
  conditionId: string,
