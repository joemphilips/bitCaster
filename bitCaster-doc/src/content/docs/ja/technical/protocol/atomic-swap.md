...
ウォレットは `PAY_TO_UNLOCK` capability で注文を認可します。注文受付では capability を確認します。受付時にミントへのネットワーク呼び出しは行いません。

認可は許可された range を対象とします。range がこの許可を超えて継続する前に、ウォレットは新しい認可を提供する必要があります。板に残る注文を取り消しても、その注文だけを取り消します。capability の使用や返金は行いません。
