...
マーケット詳細のトレードチケットは枚数ベースです。ユーザーは整数の枚数を入力し、送信前にコスト内訳を確認します。内訳は、マッチする注文額を示す **Quote payment**、推定mint手数料を示す **Est. settlement fee**、そして合計の **Total** です。現在のUIでは、受取額や実行可能枚数を別行として表示しません。ウォレット残高が足りない場合、トップアップ操作は **Top up sats wallet** と表示します。カテゴリカルマーケットは `D=10000`（1枚 = **10 sats**）、数値マーケットは `D=1000000` を使います。価格分子 `k` は `k / D` の確率を意味します。すべてのマーケット担保は **msat** で会計されます。
