...
マーケット詳細のトレードチケットは枚数ベースです。ユーザーは整数の枚数を入力し、送信前に合計コストを確認します。現在のUIでは、受取額や実行可能枚数を別行として表示しません。現在のマーケットは `D=10000` の価格精度を使うため、価格分子 `k` は `k / 10000` の確率を意味し、**53.27%** のように小数第2位までのパーセントで表示されます。satマーケットの1枚は勝った場合 **1000 sats**、USDマーケットの1枚は勝った場合 **$1.00** を受け取ります。satマーケットの担保は **msat**、USDマーケットの担保は **milli-cent** で会計されるため、USD 1枚あたり $0.5327 のようなコストも正確に表せます。
