...
bitCasterでは、ミントがビットコイン（sats）建ての通常ecashトークンを発行します。一般的なCashuミントは他の単位を広告できますが、bitCasterのウォレット画面が受け入れるのはsat入金とmsatの条件付きproofだけです。Lightning経由でトップアップするか、既存のsat Cashuトークンをトップアップ画面に貼り付けて、新しいLightningインボイスを作らずに取引用ウォレットへ直接クレジットできます。賭けを行うと、そのecashを特定の結果にロックされた**条件付きトークン**と交換します。勝てば、その条件付きトークンは通常のecashに償還可能になり、いつでもLightningに出金できます。
...
トップアップボタンには、取引に必要なウォレット単位として **Top up sats wallet** と表示されます。通常のecashをウォレットへ移すときはsatトークンを貼り付けてください。マーケットポジションに使う条件付きproofはmsat単位で、トップアップではなく取引フローが管理します。
