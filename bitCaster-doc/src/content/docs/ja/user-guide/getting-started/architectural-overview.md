...
オラクルはアナウンスメントとアテステーションをNostrリレーへ公開します。これらのリレーは信頼されない公開ネットワークとして機能します。イベントを配送・キャッシュ・遮断することはできますが、無効なアナウンスメントやアテステーションを有効にすることはできません。マッチングエンジンはこのネットワークを署名済みDLCオラクルデータの取得元として扱い、受け取った内容を信頼する前に署名を検証します。
