...
export const ENCRYPTED_WALLET_BACKUP_NIP98_ACCOUNT_PROFILE =
  "nip98-backup-intent-v1";
...
export function createEncryptedWalletBackupNip98AccountAuthorizationPort(input: {
  ...
  const port: EncryptedWalletBackupAccountAuthorizationPort = {
    async authorizeBackupAccountOperation(
      ...
      const createdAtUnixSeconds = requireTimestamp(
        clock(),
        "account authorization issue time",
      );
...
export function authenticateEncryptedWalletBackupNip98AccountRequest(input: {
  ...
  try {
    const request = decodeEncryptedWalletBackupAccountRequest(
...
function encodeCanonicalSignedEvent(
  value: EncryptedWalletBackupSignedNostrEvent,
): Uint8Array {
  const event = requireSignedEvent(value);
...
function requireSignedEvent(
  ...
  return Object.freeze({
    ...
    createdAtUnixSeconds: requireTimestamp(
