...
export function requireEncryptedWalletBackupCycleSignal(callerSignal: AbortSignal): AbortSignal {
  return requireAbortSignal(callerSignal)
}

export function throwIfEncryptedWalletBackupCycleAborted(signal: AbortSignal): void {
...
function requireAbortSignal(value: unknown): AbortSignal {
  ...
  try {
    ...
    if (typeof aborted !== 'boolean') throw new Error('encrypted backup abort signal is invalid')
  } catch {
    throw new Error('encrypted backup abort signal is invalid')
  }
