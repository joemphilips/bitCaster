...
export async function prepareEncryptedWalletBackupAccountOperation(input: {
  ...
  if (
    typeof input.authorizationPort !== 'object' ||
    input.authorizationPort === null ||
...
const ACCOUNT_OPERATION_RESULTS = new WeakMap<
  object,
...
export async function executeEncryptedWalletBackupAccountOperation(input: {
  ...
  if (response.status === 'committed') {
    ...
    const expectedLifecycle =
      authority.action === 'enroll'
        ? 'active'
        : authority.action === 'revoke'
          ? 'revoked'
          : 'deleted'
  ...
  try {
    returned = await input.store.commitAccountOperationResult(record, (raw) => {
      if (!callbackOpen || callbackCalls++ !== 0)
        throw new Error('account result callback is invalid')
...
function decodeAccountOperationResult(
  value: unknown,
): EncryptedWalletBackupAccountOperationResultRecord {
  ...
  return Object.freeze({
    ...
    result:
      raw.result === 'committed' || raw.result === 'conflict'
        ? raw.result
        : (() => {
            throw new Error('backup account result kind is invalid')
          })(),
  })
}
...
function requireExactHttpsUrl(value: unknown): string {
  if (
    ...
    throw new Error('backup account URL is invalid')
  let parsed: URL
  try {
