...
export function decodeDurableWalletStorageClassification(
  ...
  const reasons = requireArray(record.pinReasons, 'wallet storage pin reasons').map(
    requirePinReason,
  )
  ...
  const backupBinding =
    record.backupBinding === null ? null : decodeProofBackupBinding(record.backupBinding)
  const recordId =
...
function requirePreparedSnapshot(value: unknown): PreparedDurableWalletAcknowledgedBackupSnapshot {
  ...
  if (prepared[PREPARED_BACKUP_SNAPSHOT] !== true || !PREPARED_BACKUP_REACHABILITY.has(prepared)) {
