...
export async function rehydrateFinalizedEncryptedWalletBackupUploadSet(input: {
  ...
  const uploadAttemptId = requireLowerHex(
  ...
  let returned: unknown;
  try {
    returned = await input.store.readFinalizedUploadAttempt(
      uploadAttemptId,
      (raw) => {
        if (!callbackOpen || callbackCalls++ !== 0) {
  ...
  if (
    ...
    throw new Error(
      "finalized backup upload read must be synchronous and exact",
    );
  }
  return issued;
}
...
function issueFinalizedZeroDeltaRecord(
  ...
  if (
    ...
    bytesToHex(
      requireBytes(decodedHead[4], 32, "zero-delta request public key"),
...
function issueFinalizedUploadSet(
  ...
  for (const record of records) {
    ...
    if (
      ...
      throw new Error(
        "backup upload batches belong to different local snapshots",
      );
    }
...
function requireUploadStore(
  ...
  if (
    ...
    typeof (value as EncryptedWalletBackupUploadBatchStore)
      .finalizeUploadAttempt !== "function" ||
    typeof (value as EncryptedWalletBackupUploadBatchStore)
      .readFinalizedUploadAttempt !== "function"
  ) {
