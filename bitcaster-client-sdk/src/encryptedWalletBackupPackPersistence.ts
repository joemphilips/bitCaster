...
import {
  ...
  rehydratePreparedEncryptedWalletBackupDataObject,
  type EncryptedWalletBackupKeyHandle,
  type EncryptedWalletBackupRuntime,
  type PreparedEncryptedWalletBackupDataChunk,
...
export const ENCRYPTED_WALLET_BACKUP_PACK_TRANSACTION_MAX_BYTES =
  1_048_576 as const;
export const ENCRYPTED_WALLET_BACKUP_PACK_PERSISTED_ROW_MAX_BYTES =
  900_000 as const;
...
export interface EncryptedWalletBackupPackPersistenceTransaction {
  readBuildCursor(
    buildId: string,
  ): PersistedEncryptedWalletBackupBuildCursor | null;
  readPackControl(
    buildId: string,
    packId: string,
  ): PersistedEncryptedWalletBackupPackControl | null;
  readPackRecordPage(
    buildId: string,
    packId: string,
...
const preparedPackObjectAuthorities = new WeakMap<
  object,
  PreparedPackObjectAuthority
>();
...
export function measureEncryptedWalletBackupPackTransaction(input: {
  ...
  for (const row of [...input.readRows, ...input.writtenRows]) {
    if (!(row instanceof Uint8Array))
      throw new Error("backup pack transaction row is invalid");
    total += row.byteLength;
    if (total > ENCRYPTED_WALLET_BACKUP_PACK_TRANSACTION_MAX_BYTES)
      throw new Error(
        "backup pack transaction exceeds the aggregate serialized byte limit",
      );
  }
  return total;
}
...
async function exactVersionTransaction<T>(
  ...
  const pending = store.withExactVersionTransaction(expected, (transaction) => {
    ...
    if (isThenable(value))
      throw new Error("backup pack transaction callback must be synchronous");
    return sentinel;
  });
  synchronous = false;
...
> {
  ...
  const expectedPackVersion = requireVersion(
...
> {
  ...
  const chunk = packEncryptedWalletBackupDataChunk(candidate.records);
  ...
  const result = await exactVersionTransaction(
    ...
    (rawTransaction) => {
      ...
      if (pack.recordCount !== seal.recordCount)
        throw new Error("backup frozen pack count changed");
  ...
  const frozenPack = issueFrozenPack(
    input,
    chunk,
    seal,
    candidate.pageReadCount,
  );
...
export async function prepareEncryptedWalletBackupFrozenPackObjectImmediately(input: {
  ...
  if (authority === undefined)
    throw new Error("frozen backup pack authority is invalid");
...
export async function prepareEncryptedWalletBackupFrozenPackObject(input: {
  ...
  return issuePreparedPackObject(
    input,
    chunk,
    object,
    seal,
...
export async function rehydrateEncryptedWalletBackupStagedPackObject(input: {
  ...
  const object = await rehydratePreparedEncryptedWalletBackupDataObject({
    keyHandle: input.keyHandle,
    seed: input.seed,
    chunk,
...
function stagedCandidate(
  ...
  if (wire.kindCode !== ENCRYPTED_WALLET_BACKUP_DATA_CHUNK_KIND)
...
function commitStage(
  ...
  const build = requireExistingBuild(
    transaction.readBuildCursor(authority.buildId),
    stateInput,
  );
  const pack = requireExistingPack(
  ...
  if (pack.state === "staged")
    return requireIdempotentStage(transaction, build, pack, candidate);
  if (
    transaction.readStagedObject(authority.buildId, authority.packId) !== null
  )
...
function commitAppend(
  ...
  const appendedPersistedRowBytes = appendPersistedRowBytes(
    input,
    pack.recordCount,
  );
  ...
  const nextPack = Object.freeze({
    ...
    recordCanonicalBytes:
      ...
      input.records.reduce(
        (sum, { prepared }) => sum + prepared.canonicalRecord.byteLength,
        0,
      ),
  ...
  const exactNextPack = Object.freeze({
    ...nextPack,
    canonicalBytes: canonicalChunkBytes(
      nextPack.recordCount,
      nextPack.recordCanonicalBytes,
    ),
  });
  transaction.writeBuildCursor(nextBuild);
...
function requireAppendCapacityAndOrder(
  ...
  if (
    pack.recordCount + records.length >
    ENCRYPTED_WALLET_BACKUP_RECORD_COUNT_MAX
  )
...
function insertAppendRows(
  ...
  for (let index = 0; index < input.records.length; index += 1) {
    ...
    transaction.insertPackBinding(
      packBinding(record, input.packId, firstOrdinal + index),
    );
  }
}
...
function persistedAppendRowBytes(
  ...
  return (
    ...
    serializeEncryptedWalletBackupPackBinding(
      packBinding(record, packId, ordinal),
    ).byteLength
  );
}
...
class AccountedTransaction {
  ...
  readPackRecordPage(
    ...
    const remainingBytes =
      ENCRYPTED_WALLET_BACKUP_PACK_TRANSACTION_MAX_BYTES - this.totalBytes;
...
function snapshotAppendRecords(
  ...
  if (
    source.length === 0 ||
    source.length > ENCRYPTED_WALLET_BACKUP_PACK_APPEND_RECORD_MAX
  )
    throw new Error("backup pack append record count is invalid");
  ...
  for (const raw of source) {
    ...
    if (
      persistedRowBytes > ENCRYPTED_WALLET_BACKUP_PACK_PERSISTED_ROW_MAX_BYTES
    )
      throw new Error("backup pack persisted rows exceed the limit");
    records.push(record);
  }
...
function requireAppendRawBytes(
  raw: PersistedPreparedEncryptedWalletBackupRecord,
): number {
  if (
    typeof raw !== "object" ||
    raw === null ||
...
function requirePreflightCanonicalCapacity(
  recordCount: number,
  recordCanonicalBytes: number,
) {
  if (
...
function appendValidatedPackPage(
  ...
  if (page.rows.length !== expectedCount)
    throw new Error("backup pack record page is short");
...
function requireCompletePackRows(
  ...
  const persistedRowBytes = rows.reduce(
    (sum, row) =>
      ...
      serializeEncryptedWalletBackupPreparedBuildRecord(row.prepared)
        .byteLength,
    0,
  );
...
function issuePreparedPackObject(
  ...
  seal: Pick<
    PreparedPackObjectAuthority,
    "recordCount" | "canonicalBytes" | "membershipDigest"
  >,
  pageReadCount: number,
) {
...
function requireExpectedOpenPack(
  ...
  if (
    result.snapshotId !== first.snapshotId ||
...
function requireClosedBuild(build: PersistedEncryptedWalletBackupBuildCursor) {
  if (build.openPackId !== null)
    throw new Error("backup build cursor still has an open pack");
}
...
function requireExistingPack(
  ...
  const result = requirePackControl(
    row,
    input.buildId,
    input.packId,
    scopeFromInput(input),
  ...
  if (!states.includes(result.state))
    throw new Error(`backup pack is not ${states.join(" or ")}`);
  return result;
}
...
function requirePreparedPackObject(
...
function requireStagedObject(
  ...
  const digest = bytesToHex(
    sha256(
      concatBytes(uint32Bytes(value.aad.byteLength), value.aad, value.body),
    ),
  );
  if (digest !== value.digest)
    throw new Error("persisted backup staged object digest is invalid");
  return Object.freeze({
    ...value,
    aad: value.aad.slice(),
  ...
  return Object.freeze(
    value.rows.map((row) =>
      Object.freeze({
        ...
        prepared: deserializeEncryptedWalletBackupPreparedBuildRecord(
          row.prepared,
        ),
      }),
    ),
...
export function deserializeEncryptedWalletBackupPackBinding(
  ...
  return Object.freeze(
    requireBinding({
      ...
      snapshotRevision: requireVersion(
        value[7],
        "serialized snapshot revision",
      ),
  ...
  return Object.freeze(
    requirePreparedBuildRecord({
      ...
      prepared: {
        ...
        vaultId: requireFingerprintValue(
          value[10],
          "serialized prepared vault id",
        ),
        snapshotId: requireIdentifier(
          value[11],
          "serialized prepared snapshot id",
        ),
        snapshotRevision: requireVersion(
          value[12],
          "serialized prepared snapshot revision",
        ),
        recordId: requireFingerprintValue(
          value[13],
          "serialized prepared record id",
        ),
        commitment: requireFingerprintValue(
          value[14],
          "serialized prepared commitment",
        ),
...
function requireBytesValue(value: unknown): Uint8Array {
  if (!(value instanceof Uint8Array))
    throw new Error("backup pack canonical byte value is invalid");
  return value;
}
...
function requirePackControl(
  ...
  if (
    ...
    row.recordCount > ENCRYPTED_WALLET_BACKUP_RECORD_COUNT_MAX ||
    ...
    row.persistedRowBytes >
      ENCRYPTED_WALLET_BACKUP_PACK_PERSISTED_ROW_MAX_BYTES ||
    row.canonicalBytes !==
      canonicalChunkBytes(row.recordCount, row.recordCanonicalBytes) ||
...
function requirePreparedBuildRecord(
  row: PersistedEncryptedWalletBackupPreparedBuildRecord,
) {
  if (
    !hasExactKeys(row, [
...
function legalPackControlFields(
  row: PersistedEncryptedWalletBackupPackControl,
) {
  const lastRecordValid =
    row.recordCount === 0
      ? row.lastRecordId === null
      : isFingerprint(row.lastRecordId);
...
function requireRecordKindCode(value: unknown): 0 | 1 | 2 {
  if (value !== 0 && value !== 1 && value !== 2)
    throw new Error("backup pack record kind is invalid");
  return value;
}
...
function hexBytes(value: string) {
  if (!isFingerprint(value))
    throw new Error("backup pack fingerprint is invalid");
