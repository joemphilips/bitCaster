...
export const ENCRYPTED_WALLET_BACKUP_AUTHORIZATION_HEADER_MAX_CHARACTERS =
  5_471 as const;
...
export interface DecodedEncryptedWalletBackupRequestProofClaims
  extends EncryptedWalletBackupRequestProof {}
...
export type EncryptedWalletBackupDelegatedServerRequestVerificationInput =
  Omit<
...
interface VerifiedDelegatedRequestContext
  extends ValidatedDelegatedRequestContext {
...
interface EnrollmentAuthorizedDelegatedRequestAuthority
  extends VerifiedAndDecodedDelegatedRequestAuthority {
  readonly discovery:
    | Readonly<{ status: "active"; enrollmentEpoch: number }>
    | Readonly<{ status: "not-enrolled" }>
    | undefined;
...
export function decodeEncryptedWalletBackupAuthorizationHeader(
  ...
  try {
    ...
    throw new Error(
      "encrypted backup authorization header is invalid; request proof is invalid",
    );
  }
}
...
export async function authenticateAndDecodeEncryptedWalletBackupDelegatedServerRequest(
  ...
  const verifiedRequest =
    verifyAndDecodeEncryptedWalletBackupDelegatedServerRequest(input);
...
export function verifyAndDecodeEncryptedWalletBackupDelegatedServerRequest(
  input: Readonly<
    EncryptedWalletBackupDelegatedServerRequestVerificationInput
  >,
): VerifiedEncryptedWalletBackupDelegatedServerRequest {
  const context = requireSelfAuthenticatedProof(
    input,
    requireDelegatedRequestContext(input),
  );
  ...
  VERIFIED_DELEGATED_REQUESTS.set(
    verifiedRequest,
    Object.freeze({ context, decodedPayload }),
  );
...
export async function authenticateEnrollmentAuthorizedEncryptedWalletBackupDelegatedServerRequest(
  input: Readonly<
    EncryptedWalletBackupEnrollmentAuthorizedDelegatedServerRequestInput
  >,
): Promise<AuthenticatedAndDecodedEncryptedWalletBackupDelegatedServerRequest> {
  const authority = ENROLLMENT_AUTHORIZED_DELEGATED_REQUESTS.get(
    input.authorizedRequest,
  );
  if (authority === undefined) {
    throw new Error(
      "encrypted backup enrollment-authorized delegated request is invalid",
    );
  }
...
function decodeAuthorizationHeaderUnchecked(
  ...
  const decoded = decodeCanonicalBase64Url(
    value.slice(AUTHORIZATION_PREFIX.length),
  );
  return requireBytes(
    decoded,
    1,
...
function decodeRequestProofClaimsUnchecked(
  ...
  return Object.freeze({
    ...
    requestAuthPublicKey: bytesToHex(
      requireValidXOnlyPublicKey(decoded[4], "request public key"),
    ),
    ...
    replayNonce: bytesToHex(
      requireBytes(decoded[10], 16, 16, "request replay nonce"),
    ),
    ...
    payloadDigest: bytesToHex(
      requireBytes(decoded[12], 32, 32, "request payload digest"),
    ),
    signature: bytesToHex(
      requireBytes(decoded[13], 64, 64, "request signature"),
    ),
  });
}
...
function requireDelegatedRequestContext(
  ...
  try {
    ...
    const maximumPayloadBytes =
      encryptedWalletBackupDelegatedPayloadMaximumBytes(route.operation);
...
async function authenticateDelegatedProof(
  ...
  Awaited<ReturnType<typeof consumeEncryptedWalletBackupVerifiedRequestReplay>>
> {
  try {
...
function validateServerRoute(
  value: EncryptedWalletBackupServerRoute,
  ...
  switch (value.operation) {
    ...
    case "head-cas":
      return route(
        ...
        vaultId,
        "POST",
        `${base}/head:compare-and-swap`,
      );
    case "object-get":
    case "object-put":
    case "object-delete": {
      const objectId = requireLowerHex(
        value.routeObjectId,
        16,
        "route object id",
      );
    ...
    case "upload-attempt-abort": {
      const attemptId = requireLowerHex(
        value.routeAttemptId,
        16,
        "route attempt id",
      );
      return route(
...
function enrollmentMatches(
  ...
  try {
    ...
    return (
      ...
      requireLowerHex(enrollment.vaultId, 32, "enrolled vault id") ===
        claims.vaultId &&
      bytesToHex(
        requireValidXOnlyPublicKey(
          hexToBytesStrict(
            enrollment.requestAuthPublicKey,
            32,
      ...
      requireInteger(enrollment.enrollmentEpoch, 1, "enrolled epoch") ===
