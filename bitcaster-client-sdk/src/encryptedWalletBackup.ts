...
export class EncryptedWalletBackupRemoteBackoffError extends EncryptedWalletBackupRemoteFailureError {
  ...
  constructor(
    status: EncryptedWalletBackupRemoteBackoffStatus,
    ...
    if (status === "quota-exceeded" && retryAfterSeconds != null) {
      throw new Error(
        "encrypted backup quota backoff must not include retry-after",
      );
    }
    if (
...
export type EncryptedWalletBackupRequestMethod =
  | "GET"
  | "PUT"
  | "POST"
  | "DELETE";
...
const AUTHENTICATED_BACKUP_REQUESTS = new WeakMap<
  object,
...
const VERIFIED_CONDITIONAL_KEYSETS = new WeakMap<
  object,
...
const COMMITTED_SNAPSHOT_SEALS = new WeakMap<
  object,
  EncryptedWalletBackupCommittedSnapshotSeal
>();
...
const TRANSACTION_PROOF_SNAPSHOTS = new WeakMap<
  object,
  TransactionProofSnapshotAuthority
>();
...
const PREPARED_PROOF_AUTHORITIES = new WeakMap<
  object,
...
const PREPARED_CHUNK_AUTHORITIES = new WeakMap<
  object,
  PreparedChunkAuthority
>();
...
const AUTHENTICATED_HEAD_OBSERVATIONS = new WeakMap<
  object,
...
>();
const AUTHENTICATED_MANIFEST_HEAD_VALUES = new WeakMap<
  object,
  AuthenticatedHeadAuthority
>();
...
const PREPARED_MANIFEST_HEADS = new WeakMap<
  object,
...
const DECRYPTED_MANIFEST_PAGE_AUTHORITIES = new WeakMap<
  object,
...
export async function createEncryptedWalletBackupKeyHandle(input: {
  ...
  const requestScalar = await deriveRequestAuthScalar(
    runtime.subtle,
    requestAuthRoot,
    realm,
  );
  const requestAuthPublicKey = bytesToHex(
    secp256k1.getPublicKey(requestScalar, true).slice(1),
  );
...
async function prepareEncryptedWalletBackupRequestProofForEpoch(
  ...
  const expiresAt = requireNonNegativeSafeInteger(
...
export function verifyEncryptedWalletBackupRequestProofEvidence(input: {
  ...
  const now = requireNonNegativeSafeInteger(
    input.serverNowUnixSeconds,
    "server time",
  );
  if (
...
function verifiedRequestProofMatchesIdentity(
  ...
  return (
    ...
    proof.vaultId ===
      requireLowerHex(expectedVaultId, 32, "expected vault id") &&
    proof.requestAuthPublicKey ===
      requireLowerHex(expectedPublicKey, 32, "expected public key") &&
    proof.enrollmentEpoch ===
      requireInteger(
...
export function verifyEncryptedWalletBackupConditionalKeyset(input: {
  ...
  const outcomeLabel = requireBoundedText(
    input.outcomeLabel,
    256,
    "conditional outcome label",
  );
  ...
  if (finalExpiry <= registeredAt)
    throw new Error("conditional final expiry is invalid");
  const metadata = requireRecord(
    input.conditionalMetadata,
    "conditional keyset metadata",
  );
  ...
  const conditionId = requireLowerHex(
  ...
  const inputFee =
    keys.input_fee_ppk === undefined
      ...
      : requireInteger(
          ...
          0,
          2_147_483_647,
          "conditional input fee",
        );
  if (keys.active !== undefined)
    requireBoolean(keys.active, "conditional active marker");
  if (keys.conditional !== undefined) {
    const embedded = requireRecord(
      keys.conditional,
      "embedded conditional metadata",
    );
...
async function requireAuthoritativeStorageSnapshot(
  ...
  if (snapshot === undefined)
    throw new Error("authoritative storage snapshot is invalid");
  ...
  if (input.proofKind === "ctf") {
    ...
    if (
      ...
      conditional.registeredAtUnixSeconds !==
    ...
    snapshot.conditionalKeyset !== null ||
    row.conditionalKeysetEvidence !== null
  ) {
...
function requireProofSnapshotStore(
  ...
  if (
    typeof value !== "object" ||
    value === null ||
    typeof (value as { withCommittedProofSnapshot?: unknown })
      .withCommittedProofSnapshot !== "function"
  ) {
...
async function requireCommittedSnapshotSeal(
  ...
  if (
    ...
    storeInput === null ||
    typeof (storeInput as { sealCommittedBackupSnapshot?: unknown })
      .sealCommittedBackupSnapshot !== "function"
  ) {
...
function decodeCommittedSnapshotSeal(
  value: unknown,
): EncryptedWalletBackupCommittedSnapshotSeal {
...
function decodeCommittedProofSnapshot(
  value: unknown,
): EncryptedWalletBackupCommittedProofSnapshot {
  ...
  if (row.schemaVersion !== 1)
    throw new Error("unsupported committed proof snapshot version");
  ...
  return Object.freeze({
    ...
    ambiguousMintOperation: requireBoolean(
      row.ambiguousMintOperation,
      "stored ambiguity",
    ),
...
export async function prepareEncryptedWalletBackupObject(input: {
  ...
  for (let attempt = 0; attempt < OBJECT_ID_COLLISION_ATTEMPTS; attempt += 1) {
    ...
    try {
      if (
        input.objectIdExists !== undefined &&
        (await input.objectIdExists(candidateId))
      ) {
  ...
  try {
    ...
    const digest = bytesToHex(
      sha256(concatBytes(uint32Bytes(aad.byteLength), aad, body)),
    );
...
export async function rehydratePreparedEncryptedWalletBackupDataObject(input: {
  keyHandle: EncryptedWalletBackupKeyHandle;
  seed: Uint8Array;
  chunk: PreparedEncryptedWalletBackupDataChunk;
  ...
  try {
    ...
    const chunkAuthority = requireChunkAuthority(input.chunk);
    if (chunkAuthority.keyAuthority !== authority)
      throw new Error("foreign chunk");
    const seed = requireSeed(input.seed);
    if (!equalBytes(authority.seedDigest, sha256(seed)))
      throw new Error("foreign seed");
    const object = requireWireObject(input.object, authority);
    if (object.kindCode !== ENCRYPTED_WALLET_BACKUP_DATA_CHUNK_KIND)
      throw new Error("foreign object kind");
    const canonical = await decryptObjectFrame({
      authority,
      object,
      kindCode: ENCRYPTED_WALLET_BACKUP_DATA_CHUNK_KIND,
      frameBytes: ENCRYPTED_WALLET_BACKUP_DATA_FRAME_BYTES,
      cborMaxBytes: ENCRYPTED_WALLET_BACKUP_DATA_CBOR_MAX_BYTES,
    });
    ...
    throw new Error("persisted encrypted wallet backup data object is invalid");
  }
}
...
export async function prepareEncryptedWalletBackupManifest(input: {
  ...
  const snapshotNonce = requireBytes(
    input.snapshotNonce,
    16,
    "manifest snapshot nonce",
  ).slice();
  ...
  for (const binding of input.chunks) {
    ...
    if (
      seenObjectIds.has(binding.object.objectId) ||
      seenDigests.has(binding.object.digest)
    ) {
  ...
  if (input.emptySnapshot !== undefined) {
    snapshotId = requireBoundedText(
      input.emptySnapshot.snapshotId,
      128,
      "empty snapshot id",
    );
    snapshotRevision = requireNonNegativeSafeInteger(
  ...
  for (const entry of wireEntries) {
    ...
    if (
      ...
      measureCanonicalCbor(provisional) >
        ENCRYPTED_WALLET_BACKUP_MANIFEST_CBOR_MAX_BYTES
    ) {
      if (current.length === 0)
        throw new Error("manifest entry exceeds the encoded size limit");
  ...
  for (let pageIndex = 0; pageIndex < pageEntries.length; pageIndex += 1) {
    ...
    if (
      canonical.byteLength > ENCRYPTED_WALLET_BACKUP_MANIFEST_CBOR_MAX_BYTES
    ) {
...
export async function prepareIncrementalEncryptedWalletBackupManifest(input: {
  ...
  const snapshotId = requireBoundedText(
  ...
  const parentPages = [...input.parentPages].sort(
  ...
  for (const binding of input.chunks) {
    ...
    if (
      newObjectIds.has(binding.object.objectId) ||
      newObjectDigests.has(binding.object.digest)
    ) {
  ...
  for (const [objectId, digest] of inheritedChunkReferences) {
    if (
      !parentChunks.some(
        (reference) =>
          reference.objectId === objectId && reference.digest === digest,
      )
    ) {
      throw new Error(
        "incremental manifest inherited reference is stale or foreign",
      );
    }
  }
...
export function prepareEncryptedWalletBackupManifestHead(input: {
  ...
  const parentAuthority =
    input.parent === null ? null : requireManifestHeadAuthority(input.parent);
  ...
  const pageReferences = manifest.pages.map((object) =>
    objectReferenceWire(object),
  );
  ...
  for (const object of manifest.chunkObjects) {
    if (
      parentChunkReferences.some(
        (reference) =>
          reference.objectId === object.objectId &&
          reference.digest === object.digest,
      )
    )
      inheritedById.set(object.objectId, object.digest);
  }
  const inheritedChunkReferences = [...inheritedById].map(
    ([objectId, digest]) => ({ objectId, digest }),
  );
  ...
  if (
    canonicalReferenceSet.byteLength >
    ENCRYPTED_WALLET_BACKUP_REFERENCE_METADATA_MAX_BYTES
  ) {
  ...
  if (
    ...
    throw new Error(
      "manifest parent and target delta exceed the stored byte quota",
    );
  }
  const headCore = [
    1,
    ...
    parent === null
      ? null
      : [parent.generation, hexToBytes(parent.manifestDigest)],
...
export function readPreparedEncryptedWalletBackupManifestHead(
  ...
  const authority =
    typeof head === "object" && head !== null
      ? PREPARED_MANIFEST_HEADS.get(head)
      : undefined;
  if (authority === undefined)
    throw new Error("prepared manifest head is invalid");
  return Object.freeze({
...
export function readPreparedEncryptedWalletBackupManifestTarget(input: {
  ...
  return Object.freeze({
    ...
    canonicalInheritedReferenceSet:
      authority.canonicalInheritedReferenceSet.slice(),
  });
}
...
export async function advanceEncryptedWalletBackupSyncAttempt(input: {
  ...
  switch (input.event.type) {
    ...
    case "head-observed": {
      ...
      const observation =
        typeof input.event.observation === "object" &&
        input.event.observation !== null
      ...
      if (observed === current.targetHead.manifestDigest) {
        ...
        if (current.state === "retry-exhausted") {
          ...
          current.casAttempts < ENCRYPTED_WALLET_BACKUP_CAS_ATTEMPT_MAX
        ) {
  ...
  try {
    returned = await store.exhaustPreparedAttempt(
      ...
      requireInteger(
        ...
        1,
        3_600_000,
        "backup exhaustion delay",
      ),
      (raw) => {
        if (!open || calls++ !== 0)
          throw new Error("backup exhaustion callback is invalid");
          ...
          !equalSyncAttemptRecord(
            ...
            freezeSyncAttempt({
              ...
              retryNotBeforeUnixMilliseconds:
                committed.retryNotBeforeUnixMilliseconds,
            }),
          )
        )
...
export async function synchronizeEncryptedWalletBackupManifestHead(input: {
  ...
  const observeHead =
    async (): Promise<SealedEncryptedWalletBackupSyncAttempt> => {
      const issuedAt = requireNonNegativeSafeInteger(
  ...
  while (
    ...
    try {
      response = await awaitEncryptedWalletBackupCycle(
        input.remote.compareAndSwapCurrentHead({
          requestProof,
          canonicalCasPayload:
            privateAttempt.record.canonicalCasPayload.slice(),
          signal: cycleSignal,
        }),
        cycleSignal,
      );
    ...
    if (retryDelayMilliseconds !== undefined) {
      ...
      try {
        ...
        if (error instanceof EncryptedWalletBackupDeadlineError) throw error;
        if (!(error instanceof EncryptedWalletBackupRemoteFailureError))
          throw error;
        return attempt;
      }
    } else {
...
export async function decryptEncryptedWalletBackupManifestPage(input: {
  ...
  try {
    ...
    const pageReference = requireAuthenticatedPageReference(
      observation.head,
      object,
    );
    const canonical = await decryptObjectFrame({
    ...
    if (!equalBytes(canonical, encodeCanonical(decoded)))
...
export function acknowledgeDurableWalletBackupSnapshot(input: {
  ...
  const headAuthority = AUTHENTICATED_MANIFEST_HEAD_VALUES.get(
    observation.head,
  );
  if (
    headAuthority === undefined ||
    headAuthority.keyAuthority !== observation.keyAuthority
  ) {
    throw new Error("backup head evidence is not authenticated");
  }
...
export function deriveDurableWalletEncryptedBackupReceipt(input: {
  ...
  const proofCommitment = requireLowerHex(
    input.proofCommitment,
    32,
...
async function decryptDataChunkRecords(input: {
  ...
  const expectedDigest = bytesToHex(
    sha256(
      concatBytes(uint32Bytes(object.aad.byteLength), object.aad, object.body),
    ),
  );
...
async function decodeDataChunkRecords(
  ...
  for (let index = 0; index < records.length; index += 1) {
    ...
    if ((index + 1) % 4 === 0 && index + 1 < records.length)
      await yieldToHost();
  }
...
function decodeProofRecord(
  ...
  if (bytesToHex(deriver(counter).secret) !== secret)
  ...
  if (!equalBytes(commitment, expectedCommitment))
...
async function decryptObjectFrame(input: {
  ...
  if (!equalBytes(expectedAad, input.object.aad))
    throw new Error("foreign aad");
  const expectedDigest = bytesToHex(
    sha256(
      concatBytes(
        uint32Bytes(input.object.aad.byteLength),
        input.object.aad,
        input.object.body,
      ),
    ),
  );
  if (expectedDigest !== input.object.digest)
...
function decodeKeysetId(value: unknown): KeysetId {
  if (typeof value !== "string")
    throw new Error("backup proof keyset is invalid");
  if (/^(?:01|02)[0-9a-f]{14}$/.test(value))
    throw new Error("unresolved short modern keyset");
...
function isCashuLegacyBase64(value: string): boolean {
  if (!/^[A-Za-z0-9+/_-]+={0,2}$/.test(value) || value.length > 128)
    return false;
...
function decodeCtfMetadata(
  ...
  const conditionId = hexToBytes(
    requireLowerHex(record.conditionId, 32, "condition id"),
  );
  const outcomeLabel = requireBoundedText(
    record.outcomeLabel,
    256,
...
function requireDleq(
  ...
  if (curve === "bls12-381") {
    if (value !== undefined && value !== null)
      throw new Error("BLS proof cannot contain DLEQ");
    return null;
  }
...
function requireSignatureBytes(
  value: unknown,
  curve: KeysetId["curve"],
): Uint8Array {
...
function decodeBackupRequestProof(
  ...
  if (value instanceof Uint8Array) {
    ...
    value = {
      ...
      requestAuthPublicKey: bytesToHex(
        requireBytes(decoded[4], 32, "request public key"),
      ),
      ...
      payloadDigest: bytesToHex(
        requireBytes(decoded[12], 32, "payload digest"),
      ),
  ...
  return Object.freeze({
    ...
    requestAuthPublicKey: requireLowerHex(
      raw.requestAuthPublicKey,
      32,
      "request public key",
    ),
    enrollmentEpoch: requireInteger(
...
function requirePreparedObjectAuthority(
  value: unknown,
): PreparedObjectAuthority {
  if (typeof value !== "object" || value === null)
    throw new Error("prepared backup object is invalid");
...
function requirePreparedManifest(
  ...
  if (
    authority === undefined ||
    requireKeyAuthority(authority.keyHandle) !== keyAuthority
  ) {
    throw new Error("prepared manifest is invalid");
  }
...
function requireManifestHeadAuthority(
  value: unknown,
): PreparedManifestHeadAuthority {
  if (typeof value !== "object" || value === null) {
    throw new Error("manifest head is not authenticated");
  ...
  const authority =
    PREPARED_MANIFEST_HEADS.get(value) ??
    AUTHENTICATED_MANIFEST_HEAD_VALUES.get(value);
  if (authority === undefined)
    throw new Error("manifest head is not authenticated");
  return authority;
}
...
function decodeManifestHeadWire(
  ...
  if (
    ...
    backupPublicKey !==
      requireLowerHex(expectedPublicKey, 32, "expected backup public key")
  ) {
...
function requireAuthenticatedPageReference(
  ...
  for (let index = 0; index < pages.length; index += 1) {
    ...
    if (
      bytesToHex(requireBytes(reference[0], 16, "manifest page object id")) ===
        object.objectId &&
      bytesToHex(requireBytes(reference[1], 32, "manifest page digest")) ===
        object.digest
    ) {
      if (matchedIndex !== -1)
        throw new Error("manifest page reference is duplicated");
      matchedIndex = index;
    }
  }
  if (matchedIndex === -1)
...
function requireAuthenticatedChunkReference(
  ...
  for (const raw of referenceSet[3]) {
    ...
    if (
      bytesToHex(requireBytes(raw[0], 16, "manifest chunk object id")) ===
        objectId &&
...
function decodeSyncAttemptRecord(
  value: unknown,
  ...
  if (record.schemaVersion !== 1)
    throw new Error("unsupported backup sync attempt version");
  ...
  const casPayloadDigest = requireLowerHex(
    record.casPayloadDigest,
    32,
    "CAS payload digest",
  );
...
function decodePersistedManifestHeadSummary(
  value: unknown,
): EncryptedWalletBackupManifestHead {
  ...
  if (head.parent === null) {
    if (generation !== 1)
      throw new Error("persisted manifest parent is invalid");
    parent = null;
  } else {
  ...
  const backupPublicKey = requireLowerHex(
    head.backupPublicKey,
    32,
    "backup public key",
  );
  const manifestDigest = requireLowerHex(
    head.manifestDigest,
    32,
    "manifest digest",
  );
  ...
  return Object.freeze({
    ...
    storedBytes: requireNonNegativeSafeInteger(
      head.storedBytes,
      "stored bytes",
    ),
    recordCount: requireInteger(
...
async function persistSyncAttemptTransition(
  ...
  let returned: unknown;
  try {
    returned = await store.transitionPreparedAttempt(
      expected,
      next,
      (rawRecord) => {
        if (!callbackOpen || callbackCalls++ !== 0)
...
async function allocateObjectId(
  ...
  runtime: EncryptedWalletBackupRuntime,
  objectIdExists:
    | ((objectId: string) => boolean | Promise<boolean>)
    | undefined,
...
async function hkdf(
  ...
  const key = await subtle.importKey("raw", asArrayBuffer(ikm), "HKDF", false, [
    "deriveBits",
  ]);
...
function cashuSecretDeriver(seed: Uint8Array, keysetId: string): SecretDeriver {
  ...
  if (typeof create !== "function")
    throw new Error("cashu derivation is unavailable");
...
function decodeProofPins(value: unknown): EncryptedWalletBackupProofPins {
  ...
  return Object.freeze({
    openOrderCollateral: state(
      pins.openOrderCollateral,
      "stored collateral pin",
    ),
