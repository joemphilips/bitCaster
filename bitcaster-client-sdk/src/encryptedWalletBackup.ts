...
export const ENCRYPTED_WALLET_BACKUP_MANIFEST_TOTAL_ENTRY_COUNT_MAX = 512 * 1_024
export const ENCRYPTED_WALLET_BACKUP_VAULT_STORED_BYTES_MAX = 64 * 1_024 * 1_024
...
const SEALED_BACKUP_SYNC_ATTEMPTS = new WeakMap<object, SealedSyncAttemptAuthority>()
...
export const ENCRYPTED_WALLET_BACKUP_REFERENCE_METADATA_MAX_BYTES = 65_536 as const
...
function requireProofKind(value: unknown): EncryptedWalletBackupProofKind {
  if (
    !['ordinary', 'ctf-active', 'ctf-expired', 'p2pk', 'htlc', 'unknown'].includes(value as string)
  ) {
  ...
  try {
    returned = await input.store.resumeRetryExhaustedAttempt(
      ...
      (rawRecord) => {
        ...
        const evidence = authorizeSyncAttempt(committed, authority.keyAuthority)
        issued = evidence
        return evidence
      },
    )
...
export async function rehydrateEncryptedWalletBackupSyncAttempt(input: {
  ...
  try {
    returned = await input.store.readPreparedAttempt(attemptId, (rawRecord) => {
      ...
      if (record.attemptId !== attemptId) throw new Error('backup attempt id does not match')
      requireFinalizedEncryptedWalletBackupUploadSet(
        input.finalizedUploads,
        input.keyHandle,
      ...
      validateSyncAttemptCasPayload(record, keyAuthority, input.keyHandle.requestAuthPublicKey)
...
export async function advanceEncryptedWalletBackupSyncAttempt(input: {
    ...
    case 'head-observed': {
      ...
      const observation =
        typeof input.event.observation === 'object' && input.event.observation !== null
          ...
          : undefined
      if (observation === undefined || observation.keyAuthority !== attemptAuthority.keyAuthority) {
  ...
  return persistSyncAttemptTransition(input.store, current, next, attemptAuthority.keyAuthority)
}
...
function requireSealedSyncAttempt(
  ...
  const authority =
    typeof value === 'object' && value !== null ? SEALED_BACKUP_SYNC_ATTEMPTS.get(value) : undefined
...
function decodeManifestHeadWire(
  ...
  const vaultId = bytesToHex(requireBytes(headValue[3], 32, 'manifest vault id'))
  const backupPublicKey = bytesToHex(requireBytes(headValue[4], 32, 'manifest public key'))
  if (
  ...
  const generation = requireInteger(headValue[5], 1, Number.MAX_SAFE_INTEGER, 'manifest generation')
  ...
  if (headValue[6] === null) {
    ...
    if (parentGeneration !== generation - 1) throw new Error('manifest parent is invalid')
    parent = Object.freeze({
      generation: parentGeneration,
      manifestDigest: bytesToHex(requireBytes(headValue[6][1], 32, 'parent digest')),
    })
  }
  const snapshotNonce = bytesToHex(requireBytes(headValue[7], 16, 'manifest snapshot nonce'))
  const pageReferences = decodeObjectReferences(headValue[8], 'manifest page references')
  const chunkReferences = decodeObjectReferences(headValue[9], 'manifest chunk references')
  if (
  ...
  for (let index = 1; index < chunkReferences.length; index += 1) {
    if (compareHex(chunkReferences[index - 1]!.objectId, chunkReferences[index]!.objectId) >= 0) {
      throw new Error('manifest chunk references are not canonical')
    }
  }
  const allIds = [...pageReferences, ...chunkReferences].map((reference) => reference.objectId)
  if (new Set(allIds).size !== allIds.length) throw new Error('manifest object id is duplicated')
  const allDigests = [...pageReferences, ...chunkReferences].map((reference) => reference.digest)
  ...
  if (
    (proofCount === 0 && (chunkReferences.length !== 0 || pageReferences.length !== 0)) ||
    (proofCount > 0 && (pageReferences.length === 0 || chunkReferences.length === 0)) ||
    chunkReferences.length < Math.ceil(proofCount / ENCRYPTED_WALLET_BACKUP_PROOF_COUNT_MAX) ||
    chunkReferences.length > proofCount ||
    pageReferences.length <
      Math.ceil(proofCount / ENCRYPTED_WALLET_BACKUP_MANIFEST_ENTRY_COUNT_MAX) ||
    pageReferences.length > proofCount
  ) {
    throw new Error('manifest proof count does not match reference bounds')
  }
  const storedBytes = requireNonNegativeSafeInteger(headValue[11], 'manifest stored bytes')
  ...
  const referenceSetDigest = bytesToHex(requireBytes(headValue[12], 32, 'reference set digest'))
  if (
    !Array.isArray(referenceValue) ||
    referenceValue.length !== 4 ||
    referenceValue[0] !== 1 ||
    referenceValue[1] !== 'reference-set' ||
    !equalBytes(encodeCanonical(referenceValue[2]), encodeCanonical(headValue[8])) ||
    !equalBytes(encodeCanonical(referenceValue[3]), encodeCanonical(headValue[9])) ||
...
function decodeSyncAttemptRecord(value: unknown): EncryptedWalletBackupSyncAttemptRecord {
  ...
  if ((state === 'sealed' && casAttempts !== 0) || (state !== 'sealed' && casAttempts === 0))
  ...
  if ((state === 'retry-exhausted') !== (retryNotBeforeUnixMilliseconds !== null)) {
...
function validateSyncAttemptCasPayload(
  ...
  const expectedDigest =
    decoded[3] === null
      ? null
      : bytesToHex(requireBytes(decoded[3], 32, 'persisted expected head digest'))
  if (
