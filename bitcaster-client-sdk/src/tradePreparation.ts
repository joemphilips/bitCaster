...
import { selectCompleteSetMergeInputs, type CompleteSetMergeInputSelection } from './ctfSplit.ts'
...
export interface PrepareBuyerSwapInputsParams {
  ...
  conditionalInputFeePpkByKeyset: Record<string, number> | (() => Promise<Record<string, number>>)
...
export async function prepareBuyerSwapInputs(
  ...
  const desiredRegularOutputSats = quotePaymentSubunits + (params.buyerLockFeeReserveSats ?? 0)
