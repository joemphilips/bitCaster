...
const PUT_PAYLOAD_MAX_BYTES = ENCRYPTED_WALLET_BACKUP_REQUEST_PAYLOAD_MAX_BYTES;
...
export function measureCanonicalBackupCbor(value: unknown): number {
  ...
  if (value instanceof Uint8Array)
    return headerLength(value.byteLength) + value.byteLength;
...
function preflightProofRecord(record: CborShape): void {
    ...
    if (
      dleq === undefined ||
      !(
        (dleq.major === 7 && dleq.value === 22) ||
        (dleq.major === 4 && dleq.value === 3)
      )
    )
    ...
    if (
      ctf === undefined ||
      !(
        (ctf.major === 7 && ctf.value === 22) ||
        (ctf.major === 4 && ctf.value === 6)
      )
    )
...
export function preflightEncryptedBackupRequestProofCbor(
  bytes: Uint8Array,
): void {
...
export function preflightEncryptedBackupHeadCbor(bytes: Uint8Array): void {
  ...
  if (
    parent === undefined ||
    !(
      (parent.major === 7 && parent.value === 22) ||
      (parent.major === 4 && parent.value === 2)
    )
  )
...
export function preflightEncryptedBackupReferenceSetCbor(
  bytes: Uint8Array,
): void {
  const root = scanBoundedEnvelope(
    bytes,
...
export function structurallyPreflightEncryptedBackupAccountRequestCbor(
  bytes: Uint8Array,
): void {
  const root = scanBoundedEnvelope(
    bytes,
...
export function preflightEncryptedBackupAccountIntentCbor(
  bytes: Uint8Array,
): void {
...
export function structurallyPreflightEncryptedBackupAttemptAbortCbor(
  bytes: Uint8Array,
): void {
...
function scanBoundedEnvelope(
  ...
  if (state.offset !== bytes.byteLength)
    throw new Error("trailing CBOR envelope");
  return root;
}

function requireObjectReferenceArray(
  ...
  for (const reference of shape.children) {
    if (reference.major !== 4 || reference.value !== 2)
      throw new Error(`${name} shape`);
...
function scanCbor(
  ...
  if (value > limits.arrayLength && depth > 1)
    throw new Error("cbor array length");
...
function readCborArgument(
  ...
  if (width === undefined || state.offset + width > bytes.length)
    throw new Error("cbor argument");
  let value = 0n;
