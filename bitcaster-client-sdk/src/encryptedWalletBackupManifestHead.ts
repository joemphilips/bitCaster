...
export function validateEncryptedWalletBackupManifestHeadUnit(
  ...
  if (
    ...
    !equalBytes(input.canonicalReferenceSet, encodeCanonicalBackupCbor(referenceSet)) ||
    !Array.isArray(head) ||
    head.length !== 13 ||
    head[0] !== 1 ||
  ...
  const vaultId = bytesToHex(requireExactBytes(head[3], 32, 'manifest vault id'))
  const backupPublicKey = bytesToHex(requireExactBytes(head[4], 32, 'manifest public key'))
  const generation = requireInteger(head[5], 1, Number.MAX_SAFE_INTEGER, 'manifest generation')
  ...
  if (head[6] === null) {
    ...
    parent = Object.freeze({
      ...
      manifestDigest: bytesToHex(requireExactBytes(head[6][1], 32, 'parent digest')),
    })
  }
  const snapshotNonce = bytesToHex(requireExactBytes(head[7], 16, 'manifest snapshot nonce'))
  const pageReferences = decodeReferences(head[8], 'manifest page references')
  const chunkReferences = decodeReferences(head[9], 'manifest chunk references')
  if (
  ...
  if (
    new Set(allReferences.map((reference) => reference.objectId)).size !== allReferences.length ||
    new Set(allReferences.map((reference) => reference.digest)).size !== allReferences.length
  ) {
  ...
  const referenceSetDigest = bytesToHex(requireExactBytes(head[12], 32, 'reference set digest'))
  if (
    !Array.isArray(referenceSet) ||
    referenceSet.length !== 4 ||
    referenceSet[0] !== 1 ||
    referenceSet[1] !== 'reference-set' ||
    !equalBytes(encodeCanonicalBackupCbor(referenceSet[2]), encodeCanonicalBackupCbor(head[8])) ||
    !equalBytes(encodeCanonicalBackupCbor(referenceSet[3]), encodeCanonicalBackupCbor(head[9])) ||
...
function decodeReferences(value: unknown, name: string): EncryptedWalletBackupObjectReference[] {
  ...
  return value.map((raw) => {
    ...
    return Object.freeze({
      objectId: bytesToHex(requireExactBytes(raw[0], 16, 'referenced object id')),
      digest: bytesToHex(requireExactBytes(raw[1], 32, 'referenced object digest')),
    })
  })
}
