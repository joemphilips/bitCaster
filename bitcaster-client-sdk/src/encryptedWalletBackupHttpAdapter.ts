...
export type EncryptedWalletBackupHttpDispatchState =
  | "not-dispatched"
  | "dispatched"
  | "uncertain";
    ...
    EncryptedWalletBackupUploadAbortRemotePort
{
  ...
  constructor(input: {
    origin: string;
    fetch?: FetchPort;
    fallbackTimeoutMilliseconds?: number;
  }) {
  ...
  > {
    return this.#withSlot(
      ...
      () => input.signal,
      async (context) => {
        const operation = requireAccountOperation(
          input.operation,
          this.#origin,
        );
        const body = requireExactPayload(
  ...
  discoverEnrollmentEpoch(input: {
    ...
    EncryptedWalletBackupEnrollmentEpochRemotePort["discoverEnrollmentEpoch"]
  > {
    return this.#withSlot(
      false,
      () => input.signal,
      async (context) => {
        ...
        const decoded = await this.#dispatchDelegated(
          context,
          "enrollment-epoch",
          request,
          {
  ...
  abortUploadAttempt(input: {
    ...
    EncryptedWalletBackupUploadAbortRemotePort["abortUploadAttempt"]
  > {
    return this.#withSlot(
      true,
      () => input.signal,
    ...
    try {
      ...
      if (input.context.signal.aborted) {
        ...
        throw transportError(
          "deadline-exceeded",
          mutationDispatchState(input.operation),
        );
      }
      throw transportError(
        "transport-failure",
  ...
  #withSlot<T>(
    ...
    if (this.#inFlight >= MAX_IN_FLIGHT) {
      return Promise.reject(
        transportError("concurrency-exhausted", "not-dispatched"),
      );
    }
    ...
    try {
      ...
      return Promise.reject(
        transportError("invalid-request", "not-dispatched"),
      );
    }
    if (callerSignal?.aborted) {
      disposeCallerSignal?.();
      return Promise.reject(
        transportError("deadline-exceeded", "not-dispatched"),
      );
    }
  ...
  switch (endpoint) {
    ...
    case "object": {
      const match = url.pathname.match(
        new RegExp(`^${escapeRegExp(base)}/objects/([0-9a-f]{32})$`),
      );
    ...
    case "upload-attempt":
      if (
        !new RegExp(
          `^${escapeRegExp(base)}/upload-attempts/[0-9a-f]{32}$`,
        ).test(url.pathname)
      ) {
        throw new Error();
      }
...
function raceVisibleDeadline<T>(
  ...
  if (signal.aborted) {
    ...
    return Promise.reject(
      transportError(
        "deadline-exceeded",
        visibleDispatchState(context, mutation),
      ),
    );
  }
