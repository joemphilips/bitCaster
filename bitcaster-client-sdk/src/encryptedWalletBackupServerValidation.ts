...
export function hexToBytesStrict(
  value: unknown,
  byteLength: number,
  name: string,
): Uint8Array {
...
export function requireDelegatedMethod(
  value: unknown,
): EncryptedWalletBackupRequestMethod {
  if (
    value !== "GET" &&
...
export function requireValidXOnlyPublicKey(
  value: unknown,
  name: string,
): Uint8Array {
...
export function requireReplayStore(
  ...
  if (
    typeof value !== "object" ||
    value === null ||
    typeof (value as { consumeReplayNonce?: unknown }).consumeReplayNonce !==
      "function"
  ) {
...
export function decodeCanonicalBase64Url(value: string): Uint8Array {
  ...
  const decoded = Uint8Array.from(decodedText, (character) =>
...
export function requireObjectAad(input: Readonly<{
  ...
  if (
    ...
    bytesToHex(requireBytes(decoded[3], 32, 32, "AAD vault id")) !==
      input.vaultId ||
    bytesToHex(requireBytes(decoded[4], 16, 16, "AAD object id")) !==
      input.objectId ||
