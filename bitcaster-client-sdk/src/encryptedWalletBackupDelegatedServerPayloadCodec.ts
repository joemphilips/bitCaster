...
export const ENCRYPTED_WALLET_BACKUP_UPLOAD_ATTEMPT_ABORT_REQUEST_MAX_BYTES =
  128 as const;
...
function decodeObjectPutUnchecked(
  ...
  const routeVaultId = requireLowerHex(
    input.routeVaultId,
    32,
    "route vault id",
  );
  const routeObjectId = requireLowerHex(
    input.routeObjectId,
    16,
    "route object id",
  );
  ...
  return Object.freeze({
    formatVersion: 1,
    uploadAttemptId: bytesToHex(
      requireBytes(decoded[2], 16, 16, "object PUT attempt id"),
    ),
...
function decodeObjectPutIdentity(decoded: readonly unknown[]): {
  ...
  return {
    ...
    vaultId: bytesToHex(
      requireBytes(decoded[5], 32, 32, "object PUT vault id"),
    ),
    objectId: bytesToHex(
      requireBytes(decoded[6], 16, 16, "object PUT object id"),
    ),
    generation: requireInteger(decoded[7], 1, "object PUT generation"),
    paddedLength: requireObjectPaddedLength(kindCode, decoded[8]),
    objectDigestBytes: requireBytes(
      decoded[9],
      32,
      32,
      "object PUT digest",
...
function decodeHeadCasUnchecked(
  ...
  const canonicalHead = requireBytes(
    decoded[4],
    1,
    65_536,
    "head CAS canonical head",
  );
  const canonicalReferenceSet = requireBytes(
    decoded[5],
    1,
    65_536,
    "head CAS reference set",
  );
  ...
  return Object.freeze({
    formatVersion: 1,
    uploadAttemptId: bytesToHex(
      requireBytes(decoded[2], 16, 16, "head CAS attempt id"),
    ),
...
function requireHeadCasBinding(input: Readonly<{
  routeRealm: string;
  routeVaultId: string;
  enrolledRequestAuthPublicKey: string;
  ...
  if (
    ...
    input.target.vaultId !==
      requireLowerHex(input.routeVaultId, 32, "route vault id") ||
    input.target.backupPublicKey !==
      requireLowerHex(
        input.enrolledRequestAuthPublicKey,
        32,
...
function decodeUploadAttemptAbortUnchecked(
  ...
  const uploadAttemptId = bytesToHex(
    requireBytes(decoded[2], 16, 16, "upload-attempt abort id"),
  );
  if (
    uploadAttemptId !==
    requireLowerHex(input.routeAttemptId, 16, "route attempt id")
  ) {
    throw new Error();
  }
  return Object.freeze({
    ...
    targetManifestDigest: bytesToHex(
      requireBytes(
        ...
        32,
        32,
        "upload-attempt abort target digest",
      ),
    ),
  });
}
...
function decodeExpectedManifestDigest(value: unknown): string | null {
  return value === null
    ? null
    : bytesToHex(requireBytes(value, 32, 32, "head CAS expected digest"));
}
...
function requireObjectPaddedLength(
  kindCode: 1 | 2,
  value: unknown,
): 65_536 | 262_144 {
  switch (kindCode) {
