...
const PREPARED_MANIFEST_UPLOADS = new WeakMap<object, PreparedManifestUploadAuthority>()
...
export function readPreparedEncryptedWalletBackupUploadAuthority(
  ...
  const authority =
    typeof head === 'object' && head !== null ? PREPARED_MANIFEST_UPLOADS.get(head) : undefined
  ...
  return Object.freeze({
    ...
    repackedSourceObjectIdsByObjectId: new Map(
      [...authority.repackedSourceObjectIdsByObjectId].map(([objectId, sourceIds]) => [
        objectId,
        new Set(sourceIds),
      ]),
    ),
