...
type AccountResponseContext<
  O extends "account-enroll" | "account-revoke" | "account-delete",
> = Readonly<{
  operation: O;
  ...
  switch (raw.kind) {
    case "account-result": {
      ...
      return encodeCanonicalBackupCbor([
        ...
        "account-result",
        hexToBytes(
          requireLowerHex(raw.operationId, 16, "account operation id"),
        ),
        hexToBytes(
          requireLowerHex(raw.intentDigest, 32, "account intent digest"),
        ),
      ...
      return encodeCanonicalBackupCbor(
        result === "active"
          ? [
              ...
              hexToBytes(
                requireLowerHex(raw.requestDigest, 32, "request digest"),
              ),
              result,
              requirePositiveInteger(raw.enrollmentEpoch, "enrollment epoch"),
            ]
    ...
    case "object-result": {
      ...
      if (
        ...
        throw new Error(
          "encrypted backup object response body length is invalid",
        );
      }
      requireObjectAad({
        aad,
      ...
      if (recomputedDigest !== objectDigest) {
        throw new Error(
          "encrypted backup object response framed digest is invalid",
        );
      }
      return encodeCanonicalBackupCbor([
        1,
...
function decodeAccountResult(
  context: AccountResponseContext<
    "account-enroll" | "account-revoke" | "account-delete"
  >,
  tuple: readonly unknown[],
): DecodedEncryptedWalletBackupHttpResponse {
  requireTupleHeader(tuple, 7, "account-result");
  const operationId = bytesToHex(
    requireExactBytes(tuple[2], 16, "account operation id"),
  );
  const intentDigest = bytesToHex(
    requireExactBytes(tuple[3], 32, "account intent digest"),
  );
  if (
    operationId !==
    requireLowerHex(context.expectedOperationId, 16, "expected operation id")
  ) {
    throw new Error("account response operation id does not match request");
  }
  if (
    intentDigest !==
    requireLowerHex(context.expectedIntentDigest, 32, "expected intent digest")
  ) {
...
function decodeEpochResult(
  ...
  if (result === "not-enrolled") {
    if (tuple.length !== 4)
      throw new Error("enrollment epoch absence tuple arity is invalid");
    return Object.freeze({
      operation: context.operation,
      result,
    });
  ...
  if (tuple.length !== 5)
    throw new Error("enrollment epoch tuple arity is invalid");
  return Object.freeze({
    operation: context.operation,
    result,
...
function decodeHeadResult(
  ...
  if (
    ...
    requirePositiveInteger(
      context.expectedEnrollmentEpoch,
  ...
  if (result === "not-found") {
    if (tuple.length !== 5)
      throw new Error("head absence tuple arity is invalid");
    return Object.freeze({
      operation: context.operation,
      result,
      enrollmentEpoch,
  ...
  const canonicalReferenceSet = requireBytes(
    tuple[6],
    1,
    65_536,
    "canonical reference set",
...
function decodeObjectResult(
  ...
  if (result === "not-found") {
    if (tuple.length !== 4)
      throw new Error("object absence tuple arity is invalid");
    return Object.freeze({
      operation: context.operation,
      result,
    });
  ...
  if (tuple.length !== 13)
    throw new Error("object result tuple arity is invalid");
  ...
  const objectDigest = bytesToHex(
    requireExactBytes(tuple[10], 32, "object digest"),
  );
  ...
  const expectedVault = requireLowerHex(
    context.expectedVaultId,
    32,
    "expected vault id",
  );
  const expectedObject = requireLowerHex(
    context.expectedObjectId,
    16,
    "expected object id",
  );
  const expectedDigest = requireLowerHex(
    context.expectedObjectDigest,
    32,
...
function decodeError(
  ...
  if (!isAllowedError(operation, status, code)) {
    throw new Error(
      "encrypted backup HTTP response status/code pair is invalid",
    );
  }
  return Object.freeze({
    operation,
...
function successResponseMaximum(
  operation: EncryptedWalletBackupHttpOperation,
): number {
  switch (operation) {
    case "account-enroll":
...
function requireConsistentManifestHeadUnit(
  ...
  if (
    input.expectedRealm !== undefined &&
    validated.realm !== requireRealm(input.expectedRealm)
  ) {
  ...
  if (
    ...
    validated.backupPublicKey !==
      requireLowerHex(
        input.expectedBackupPublicKey,
        32,
        "expected backup public key",
      )
  ) {
...
function requireObjectAad(input: {
  ...
  if (
    ...
    bytesToHex(requireExactBytes(decoded[3], 32, "AAD vault id")) !==
      input.vaultId ||
    bytesToHex(requireExactBytes(decoded[4], 16, "AAD object id")) !==
      input.objectId ||
...
function requireTuple(value: unknown): readonly unknown[] {
  if (!Array.isArray(value))
    throw new Error("encrypted backup HTTP response must be a tuple");
  return value;
}
...
function requireTupleHeader(
  ...
  if (tuple.length !== length || tuple[0] !== 1 || tuple[1] !== discriminator) {
    throw new Error(
      "encrypted backup HTTP response tuple is invalid for operation",
    );
  }
}
...
function requireKindCode(value: unknown): 1 | 2 {
  if (value !== 1 && value !== 2)
    throw new Error("encrypted backup object kind is invalid");
  return value;
}
