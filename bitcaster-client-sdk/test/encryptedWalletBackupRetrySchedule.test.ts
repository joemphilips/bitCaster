...
test('backup retry schedule freezes deterministic cross-client vectors', () => {
  assert.deepEqual(plan({ currentStreak: 0, minimumDelayMilliseconds: 5_000 }), {
    streak: 1,
    delayMilliseconds: 5_682,
  })
  assert.deepEqual(plan({ currentStreak: 1, minimumDelayMilliseconds: 5_000 }), {
    streak: 2,
    delayMilliseconds: 10_796,
  })
  assert.deepEqual(plan({ currentStreak: 0, minimumDelayMilliseconds: 17_000 }), {
    streak: 1,
...
test('persisted retry streak survives reload, grows exponentially, and caps', () => {
  ...
  assert.equal(
    plan({ currentStreak: streak, minimumDelayMilliseconds: 5_000 }).delayMilliseconds,
    ENCRYPTED_WALLET_BACKUP_RETRY_MAX_MILLISECONDS,
  )
  assert.equal(
    plan({
...
test('server backoff is never shortened and deterministic jitter stays within twenty percent', () => {
  for (const minimum of [5_000, 17_000, 60_000, 3_000_000]) {
    ...
    assert.ok(
      scheduled.delayMilliseconds <=
        Math.min(ENCRYPTED_WALLET_BACKUP_RETRY_MAX_MILLISECONDS, minimum + Math.floor(minimum / 5)),
    )
  }
})
  ...
  for (const currentStreak of [0, 4, 8]) {
    ...
    for (let index = 0; index < 4_096; index += 1) {
      ...
      const bucket = Math.min(
        buckets.length - 1,
        Math.floor(((first.delayMilliseconds - exponential) * buckets.length) / (jitterRoom + 1)),
      )
      buckets[bucket] += 1
    }
...
test('invalid retry identities and persisted bounds fail closed', () => {
  ...
  assert.throws(() => plan({ currentStreak: -1, minimumDelayMilliseconds: 5_000 }))
  assert.throws(() => plan({ currentStreak: 0, minimumDelayMilliseconds: 3_600_001 }))
  assert.throws(() =>
    planEncryptedWalletBackupRetry({
      realm: 'Production',
      vaultId: VAULT_ID,
