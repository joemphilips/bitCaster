...
const OBJECT_CONTEXTS = new Map<string, EncryptedWalletBackupHttpResponseContext>()
...
test('backup HTTP codec accepts every closed success tuple', () => {
  const cases: readonly {
    ...
    check(result: ReturnType<typeof decodeEncryptedWalletBackupHttpResponse>): void
  }[] = [
    {
      value: {
        kind: 'account-result',
    ...
    },
    {
      ...
      check: (result) => {
        ...
        assert.equal(bytesToHex(sha256(result.canonicalHead)), bytesToHex(sha256(HEAD)))
      },
    },
    {
      value: {
    ...
    },
    {
      ...
      check: (result) => {
        ...
        assert.equal(result.generation, 1, 'inherited proof chunks may predate the head')
    ...
    ...(['stored', 'already-stored'] as const).map((result) => ({
      ...
      check: (decoded: ReturnType<typeof decodeEncryptedWalletBackupHttpResponse>) =>
    ...
    ...(['abandoned', 'already-abandoned', 'already-finalized'] as const).map((result) => ({
      value: {
...
test('head GET binds one complete canonical head/reference unit to its exact context', () => {
  ...
  const rawMismatch = encode(
    [1, 'head-result', hexToBytes(REQUEST_DIGEST), 'found', 2, HEAD, mismatchedReferences],
...
test('the operation, HTTP status, error code, and retry-after matrix is closed', () => {
  ...
  for (const operation of operations) {
    assert.ok(encryptedWalletBackupHttpResponseMaximumBytes(operation, 200) > 0)
    for (const status of statusCodes) {
      assert.equal(encryptedWalletBackupHttpResponseMaximumBytes(operation, status), 128)
    }
  }
  for (const status of [199, 201, 402, 500, 504]) {
    assert.throws(() => encryptedWalletBackupHttpResponseMaximumBytes('object-get', status))
  }
...
test('object GET strictly binds AAD, identity, digest, and head generation', () => {
  ...
  if (wrongAad.kind !== 'object-result' || wrongAad.result !== 'found') throw new Error('fixture')
...
test('strict response preflight rejects hostile and noncanonical CBOR', () => {
  ...
  const wrongValues = [
    ...
    [1, 'head-result', hexToBytes(REQUEST_DIGEST), 'found', 0, HEAD, REFERENCE_SET],
  ]
...
test('account and abort requests have bounded strict CBOR preflights', () => {
  ...
  assert.doesNotThrow(() => structurallyPreflightEncryptedBackupAccountRequestCbor(account))
  assert.doesNotThrow(() => structurallyPreflightEncryptedBackupAttemptAbortCbor(abort))
  for (const body of [
    Uint8Array.of(0xa0),
    ...
    assert.throws(() => structurallyPreflightEncryptedBackupAccountRequestCbor(body))
  }
  for (const body of [
    Uint8Array.of(0x9f, 0xff),
    abort.slice(0, -1),
    ...
    assert.throws(() => structurallyPreflightEncryptedBackupAttemptAbortCbor(body))
  }
})
...
test('public JSON vectors freeze every success/error tuple and hostile case', async () => {
  const vectors = JSON.parse(
    await readFile(
      new URL('../../test-vectors/encrypted-wallet-backup-http-v1.json', import.meta.url),
      'utf8',
    ),
  ) as HttpVectors
...
function requestContext(
  operation: Exclude<
    ...
    'account-enroll' | 'account-revoke' | 'account-delete' | 'object-get' | 'head-get'
  >,
): EncryptedWalletBackupHttpResponseContext {
...
function vectorContext(item: HttpVector): EncryptedWalletBackupHttpResponseContext {
    ...
    return {
      ...
      expectedObjectDigest:
        ...
        (
          objectResponse({
            kindCode: item.objectKindCode ?? 1,
            generation: item.objectKindCode === 2 ? (item.currentHeadGeneration ?? 2) : 1,
            currentHeadGeneration: item.currentHeadGeneration ?? 2,
          }) as Extract<
            EncryptedWalletBackupHttpResponseValue,
            { kind: 'object-result'; result: 'found' }
...
function assertConstructedVectorSafe(item: ConstructedHttpVector): void {
  ...
  assert.ok(Number.isSafeInteger(item.currentHeadGeneration) && item.currentHeadGeneration >= 1)
  ...
  assert.ok(item.cborLength <= encryptedWalletBackupHttpResponseMaximumBytes('object-get', 200))
}
...
function structuredCanonicalVector(name: string): EncryptedWalletBackupHttpResponseValue | null {
  switch (name) {
