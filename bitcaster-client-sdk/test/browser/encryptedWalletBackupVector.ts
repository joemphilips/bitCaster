    ...
    globalThis.__encryptedWalletBackupVectorResult = {
      ...
      error:
        error instanceof Error
          ? error.message
          : "unknown browser vector failure",
    };
  },
);
...
async function run(): Promise<{
  ...
  const runtime = deterministicRuntime([
    fromHex(input.objectIdHex),
    fromHex(input.nonceHex),
  ]);
  ...
  equal(
    ...
    expected.requestAuthPublicKeyHex,
    "requestAuthPublicKeyHex",
  );
  ...
  equal(
    ...
    expected.requestPayloadDigestHex,
    "request payload digest",
  );
  equal(
    requestProof.signature,
    expected.requestSignatureHex,
    "request signature",
  );
  equal(
  ...
  equal(
    toHex(await digest(wire.body)),
    expected.bodySha256Hex,
    "bodySha256Hex",
  );
  ...
  equal(
    JSON.stringify(restored).includes(expected.derivedSecretHex),
    false,
    "decoded opacity",
  );
...
async function exerciseManifestVector(
  ...
  equal(
    pageWire.digest,
    vector.expected.manifestPageDigestHex,
    "manifest page digest",
  );
  equal(
    toHex(pageWire.aad),
    vector.expected.manifestPageAadHex,
    "manifest page AAD",
  );
  equal(
...
async function exerciseFailureCases(
  ...
  const noncanonical = await encryptRawFrame(
...
async function exerciseMaxLegacyRestoreScheduling(
  ...
  for (let counter = 0; counter < 512; counter += 1) {
    ...
    preparedProofs.push(
      await prepareEncryptedWalletBackupProof(await bindProofStore(input)),
    );
  }
  ...
  if (elapsedMs > 20_000)
    throw new Error(`legacy restore exceeded generous bound: ${elapsedMs}`);
...
async function encryptRawFrame(cbor: Uint8Array) {
  ...
  const digestValue = toHex(
    await digest(concat(uint32(aad.byteLength), aad, body)),
  );
  return {
