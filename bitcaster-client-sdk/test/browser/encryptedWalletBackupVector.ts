...
void run().then(
  (metrics) => { globalThis.__encryptedWalletBackupVectorResult = { ok: true, ...metrics } },
  (error: unknown) => {
...
async function run(): Promise<{ legacyRestoreMs: number; modeledChunks: number; modeledWorkSlices: number }> {
...
async function exerciseBlsAndCtf(
  ...
  const blsProof = await prepareEncryptedWalletBackupProof(await bindProofStore({
    ...blsInput,
  }))
  ...
  const blsDecoded = await decryptEncryptedWalletBackupProofChunk({
    keyHandle, seed, object: readPreparedEncryptedWalletBackupObject(blsObject),
  })
  ...
  const ctfDecoded = await decryptEncryptedWalletBackupProofChunk({
    keyHandle, seed, object: readPreparedEncryptedWalletBackupObject(ctfObject),
  })
...
async function exerciseFailureCases(
  ...
  await rejects(
    () => decryptEncryptedWalletBackupProofChunk({ keyHandle, seed, object: { ...wire, body: mutate(wire.body, 100) } }),
    'tamper',
  )
  await rejects(
    () => decryptEncryptedWalletBackupProofChunk({ keyHandle, seed, object: { ...wire, body: wire.body.slice(1) } }),
    'truncation',
  )
  ...
  await rejects(
    async () => prepareEncryptedWalletBackupProof({
      ...await bindProofStore(baseProofInput(seed, keyHandle)),
      keyHandle: { ...keyHandle },
    }),
...
async function exerciseMaxLegacyRestoreScheduling(
  ...
  const decoded = await decryptEncryptedWalletBackupProofChunk({
    keyHandle, seed, object: readPreparedEncryptedWalletBackupObject(object),
  })
...
async function bindProofStore(
  ...
  conditionalKeysetEvidence: ReturnType<typeof verifyEncryptedWalletBackupConditionalKeyset> | null = null,
) {
  const keysetKind = /^(?:01|02)[0-9a-f]{64}$/.test(input.proof.id) ? 2
    : /^00[0-9a-f]{14}$/.test(input.proof.id) ? 1 : 0
  const ctf = input.ctfMetadata === null ? null : [
    fromHex(input.ctfMetadata.conditionId), input.ctfMetadata.outcomeLabel,
    fromHex(input.ctfMetadata.outcomeCollectionId), input.ctfMetadata.registeredAtUnixSeconds,
  ...
  const commitment = toHex(await digest(encodeCanonicalBackupCbor([
    1, 'proof-record-commitment', input.mint, input.unit, [keysetKind, input.proof.id],
    input.proof.amount, new TextEncoder().encode(input.proof.secret), fromHex(input.proof.C), dleq,
  ...
  const identityKeyset = keysetKind === 0
    ? `legacy:${toHex(fromBase64(input.proof.id))}`
    : input.proof.id
  const proofId = toHex(await digest(encodeIdentity([
    'bitcaster/custody-proof-id/v1', input.mint, input.unit, identityKeyset, input.proof.secret,
  ])))
...
function deterministicRuntime(values: Uint8Array[]): EncryptedWalletBackupRuntime {
  ...
  return {
    ...
    getRandomValues(target) {
      ...
      if (value === undefined || value.byteLength !== target.byteLength) throw new Error('random vector mismatch')
      target.set(value)
      return target
    },
  }
