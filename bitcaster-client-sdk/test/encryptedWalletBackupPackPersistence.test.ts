...
test("pack persistence counts updated rows as both reads and writes at the 1 MiB boundary", () => {
  const half = new Uint8Array(
    ENCRYPTED_WALLET_BACKUP_PACK_TRANSACTION_MAX_BYTES / 2,
  );
  assert.equal(
    measureEncryptedWalletBackupPackTransaction({
      readRows: [half],
...
test("serialized page byte evidence rejects overflow before row decoding", () => {
  for (const size of [
    ...
    validateEncryptedWalletBackupPackSerializedPageByteEvidence(
      {
        rows: [
          { binding: Uint8Array.of(0), prepared: new Uint8Array(size - 1) },
        ],
...
test("exact-version transaction rejects missing, repeated, deferred, and substituted callbacks", async () => {
  ...
  for (const mode of ["never", "double", "deferred", "substituted"] as const) {
    ...
    await assert.rejects(
      appendPage(fixture, store, fixture.records, 0, 0),
      /callback|exact/,
    );
...
test("prepared snapshot batches reject missing, repeated, deferred, and substituted callbacks", async () => {
  const fixture = await preparedFixture(1);
  for (const mode of ["never", "double", "deferred", "substituted"] as const) {
    const snapshotStore = maliciousBatchSnapshotStore(
...
test("append authenticates and persists one synchronous caller-owned snapshot", async () => {
  ...
  const snapshotStore: EncryptedWalletBackupPreparedRecordSnapshotBatchStore = {
    withCommittedPreparedRecordSnapshotBatch(recordIds, read) {
      const pending =
        fixture.snapshotStore.withCommittedPreparedRecordSnapshotBatch(
          recordIds,
          read,
        );
...
test("append stores normalized rows once and exact-CAS advances compact controls", async () => {
  ...
  const second = await appendPage(
    fixture,
    store,
    fixture.records.slice(2),
    1,
    1,
  );
  ...
  await assert.rejects(
    appendPage(fixture, store, fixture.records.slice(2), 1, 1),
    /stale/,
  );
...
test("fake adapter stops canonical row acquisition at maxBytes before decoding", async () => {
  ...
  const page = (await store.withExactVersionTransaction(
    ...
    (transaction) =>
      transaction.readPackRecordPage(
        "build-a",
        "pack-a",
        null,
        256,
...
test("freeze and restart revalidate exact order, identity, count, bytes, and digest", async () => {
  ...
  for (const field of [
    ...
    if (field === "membershipDigest")
      store.pack.membershipDigest = "ff".repeat(32);
    await assert.rejects(
      preparePack(fixture, store, restartedHandle, 2, 2),
  ...
  for (const mutate of [
    ...
    store.preparedBytes.set(
      firstKey,
      serializeEncryptedWalletBackupPreparedBuildRecord(changed),
    );
    await assert.rejects(preparePack(fixture, store, restartedHandle, 2, 2));
    store.prepared.set(firstKey, exact);
    store.preparedBytes.set(
      firstKey,
      serializeEncryptedWalletBackupPreparedBuildRecord(exact),
    );
  }
});
...
test("freeze and stage failures roll back every normalized row and control", async () => {
  ...
  const prepared =
    await prepareEncryptedWalletBackupFrozenPackObjectImmediately({
...
test("512 frozen records use exactly two keyset pages and staging is immutable/idempotent", async () => {
  ...
  await assert.rejects(
    preparePack(fixture, store, fixture.keyHandle, 3, 3),
    /record page is short/,
  );
  store.shortPage = false;

  store.pageLimits.length = 0;
  const abandonedAfterEncrypt =
    await prepareEncryptedWalletBackupFrozenPackObjectImmediately({
      frozenPack: frozen.frozenPack,
      generation: 1,
...
test("fresh process rehydrates staged bytes against independent committed snapshots", async () => {
  ...
  assert.notEqual(
    runStagedRestartChild(store, fixture, { tamperCiphertext: true }).status,
    0,
  );
  assert.notEqual(
    runStagedRestartChild(store, fixture, { staleSnapshot: true }).status,
    0,
  );
});
  ...
  return {
    async withCommittedPreparedRecordSnapshot(recordId, read) {
      ...
      if (snapshot === undefined)
        throw new Error("missing prepared record snapshot");
...
function maliciousBatchSnapshotStore(
  ...
  EncryptedWalletBackupPreparedRecordSnapshotStore {
  return {
    withCommittedPreparedRecordSnapshot:
      base.withCommittedPreparedRecordSnapshot.bind(base),
    async withCommittedPreparedRecordSnapshotBatch(recordIds, read) {
      let rows:
        | readonly EncryptedWalletBackupPreparedRecordSnapshot[]
        | undefined;
      const capture = base.withCommittedPreparedRecordSnapshotBatch(
        recordIds,
        (value) => {
...
class MemoryPackStore implements EncryptedWalletBackupPackPersistenceStore {
  ...
  async withExactVersionTransaction<T>(
    ...
    const transaction: EncryptedWalletBackupPackPersistenceTransaction = {
      ...
      readPackRecordPage: (buildId, packId, afterRecordId, limit, maxBytes) => {
        ...
        for (const row of candidates) {
          ...
          const binding = working.bindingBytes.get(
            `${row.buildId}:${row.packId}:${row.recordId}`,
          )!;
          const prepared = working.preparedBytes.get(
            `${row.buildId}:${row.recordId}`,
          )!;
          if (
        ...
        if (serializedBytes > maxBytes)
          throw new Error("fake serialized page exceeds maxBytes");
      ...
      insertPreparedRecord: (row) => {
        ...
        if (working.prepared.has(key))
          throw new Error("unique build/record constraint");
        const exact = structuredClone(row);
        working.prepared.set(key, exact);
        working.preparedBytes.set(
          key,
          serializeEncryptedWalletBackupPreparedBuildRecord(exact),
        );
      ...
      insertPackBinding: (row) => {
        ...
        if (working.bindings.has(key))
          throw new Error("unique build/pack/record constraint");
        const exact = structuredClone(row);
        working.bindings.set(key, exact);
        working.bindingBytes.set(
          key,
          serializeEncryptedWalletBackupPackBinding(exact),
        );
      },
      ...
      writePackControl: (row) => {
        if (this.failOnPackWrite)
          throw new Error("injected pack-control write failure");
        working.pack = structuredClone(row);
      },
      insertStagedObject: (row) => {
        if (this.failOnStagedInsert)
          throw new Error("injected staged-object write failure");
        const key = `${row.buildId}:${row.packId}`;
        if (working.staged.has(key))
          throw new Error("unique staged object constraint");
    ...
    if (this.callbackMode === "substituted")
...
function mutatePage(
  ...
  if (mode === "duplicate" && rows.length > 1)
    rows[1] = structuredClone(rows[0]!);
  if (mode === "cross-build")
    rows[0]!.binding = { ...rows[0]!.binding, buildId: "foreign" };
  if (mode === "cross-pack")
    rows[0]!.binding = { ...rows[0]!.binding, packId: "foreign" };
  if (mode === "cross-realm")
    rows[0]!.binding = { ...rows[0]!.binding, realm: "foreign-realm" };
...
function runStagedRestartChild(
  ...
  return spawnSync(
    ...
    [
      ...
      fileURLToPath(
        new URL(
          "./encryptedWalletBackupPackPersistenceChild.ts",
          import.meta.url,
        ),
      ),
    ],
