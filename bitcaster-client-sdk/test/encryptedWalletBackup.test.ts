...
test("raw upload descriptors are not public planner authority", () => {
  assert.equal(
    "planEncryptedWalletBackupUploadBatches" in BackupSyncModule,
    false,
  );
});
...
test("account lifecycle authorization is scheme-neutral and exact-vault scoped", async () => {
  ...
  assert.ok(
    readPreparedEncryptedWalletBackupAccountOperation(enrolled).byteLength > 0,
  );
  assert.throws(
...
test("public vector freezes key derivation, canonical proof bytes, AEAD body, and restore", async () => {
  ...
  assert.equal(
    JSON.stringify(keyHandle).includes(vector.inputs.seedHex),
    false,
  );
  ...
  assert.equal(
    requestVector.payloadDigest,
    vector.expected.requestPayloadDigestHex,
  );
  ...
  assert.equal(
    isDeepStrictEqual(retry.body, wire.body),
    true,
    "retry body must be byte-exact",
  );
  assert.equal(
    isDeepStrictEqual(retry.aad, wire.aad),
    true,
...
test("manifest pages flatten interleaved immutable chunks into one sorted private index", async () => {
  ...
  assert.equal(
    toHex(headWire.canonicalHead),
    vector.expected.manifestCanonicalHeadHex,
  );
  assert.equal(
    toHex(headWire.canonicalReferenceSet),
    vector.expected.manifestCanonicalReferenceSetHex,
  );
  const manifestPageWire = readPreparedEncryptedWalletBackupObject(
    manifest.pages[0]!,
  );
  assert.equal(
    manifestPageWire.objectId,
    vector.expected.manifestPageObjectIdHex,
  );
...
test("incremental manifests and upload-ledger recovery remain exact", async () => {
  ...
  const carriedManifest = await prepareIncrementalEncryptedWalletBackupManifest(
    {
      ...
      snapshotStore: acceptingSnapshotSealStore(),
      runtime: deterministicRuntime([
        new Uint8Array(16).fill(90),
        new Uint8Array(12).fill(91),
      ]),
    },
  );
  ...
  const repackedManifest =
    await prepareIncrementalEncryptedWalletBackupManifest({
      keyHandle,
      generation: 2,
      ...
      snapshotStore: acceptingSnapshotSealStore(),
      runtime: deterministicRuntime([
        new Uint8Array(16).fill(94),
        new Uint8Array(12).fill(95),
      ]),
    });
  ...
  const repackedTarget =
    BackupModule.readPreparedEncryptedWalletBackupManifestTarget({
      keyHandle,
      head: repackedHead,
    });
  const repackedInherited = decode(
    repackedTarget.canonicalInheritedReferenceSet,
  ) as unknown[];
  ...
  const emptyChildReferences = decode(
    readPreparedEncryptedWalletBackupManifestHead(emptyChildHead)
      .canonicalReferenceSet,
  ) as unknown[];
...
test("upload ledger execution, retry, abort, and rehydration are durable", async () => {
  ...
  const uploadClaim = await uploadAttemptClaimForTest(
    keyHandle,
    head,
    uploadStore,
    "72".repeat(16),
  ...
  const uploaded = await uploadEncryptedWalletBackupBatch({
    ...
    remote: {
      async putObject({ requestProof, canonicalPutPayload }) {
        ...
        assert.equal(
          requestProof.payloadDigest,
          toHex(await sha256(canonicalPutPayload)),
        );
  ...
  const retryUploadBatch = await sealEncryptedWalletBackupUploadBatch({
    ...
    plannedBatch: plannedUploadBatchForTest(
      keyHandle,
      head,
      retryUploadClaim.record.attemptId,
    ),
  ...
  corruptedUploadRecord.items[0]!.canonicalPutPayload = Uint8Array.of(
    0x9f,
    0xff,
  );
  await assert.rejects(
  ...
  for (const [field, replacement, message] of [
    ...
    mismatch.items[0]!.payloadLength =
      mismatch.items[0]!.canonicalPutPayload.byteLength;
    mismatch.uploadedBytes = mismatch.items.reduce(
      (sum, item) => sum + item.payloadLength,
      0,
    );
    await assert.rejects(
      () =>
  ...
  futureChunk.uploadedBytes = futureChunk.items.reduce(
    (sum, item) => sum + item.payloadLength,
    0,
  );
  await assert.rejects(
  ...
  assert.equal(
    isDeepStrictEqual(
      ...
      retryUploadBatch.record.items
        .map((item) => toHex(item.canonicalPutPayload))
        .sort(),
    ),
    true,
  ...
  assert.ok(
    firstPayloads.every((payload) => retriedPayloads.includes(payload)),
  );
  ...
  const abandonedBatch = await sealEncryptedWalletBackupUploadBatch({
    ...
    plannedBatch: plannedUploadBatchForTest(
      keyHandle,
      head,
      abandonedClaim.record.attemptId,
    ),
  ...
  const tamperedPage = readPreparedEncryptedWalletBackupObject(
    manifest.pages[0]!,
  );
  ...
  corruptedHeadWire.canonicalReferenceSet[
    corruptedHeadWire.canonicalReferenceSet.length - 1
  ] ^= 1;
  await assert.rejects(
    () =>
      readAuthenticatedEncryptedWalletBackupHead({
  ...
  const unsortedHeadValue = structuredClone(
    decode(headWire.canonicalHead) as unknown[],
  );
  ...
  const unsortedReferenceBytes = encode(
    unsortedReferenceValue,
    rfc8949EncodeOptions,
  );
  ...
  const impossibleHeadValue = structuredClone(
    decode(headWire.canonicalHead) as unknown[],
  );
...
test("finalization, abort races, aggregate partitions, and CAS recovery fail closed", async () => {
  ...
  const nextManifest = await prepareIncrementalEncryptedWalletBackupManifest({
    ...
    snapshotStore: acceptingSnapshotSealStore(),
    runtime: deterministicRuntime([
      new Uint8Array(16).fill(84),
      new Uint8Array(12).fill(85),
    ]),
  });
  ...
  const foreignManifest = await prepareIncrementalEncryptedWalletBackupManifest(
    {
      ...
      snapshotStore: acceptingSnapshotSealStore(),
      runtime: deterministicRuntime([
        new Uint8Array(16).fill(87),
        new Uint8Array(12).fill(88),
      ]),
    },
  );
  ...
  const finalizedBatchAfterRestart =
  ...
  const duplicateReferenceBytes = encode(
  ...
  duplicateDigestRaw.attempt.canonicalTargetReferenceSet =
    duplicateReferenceBytes;
  ...
  const abortUncertainAfterRestart =
    await rehydrateEncryptedWalletBackupUploadBatch({
  ...
  assert.equal(
    partitionPlan.batches.reduce(
      (total, batch) => total + batch.objectCount,
      0,
    ),
    partitionPlan.objectCount,
  );
  assert.equal(
    partitionPlan.batches[0]!.objectCount,
    ENCRYPTED_WALLET_BACKUP_CYCLE_REQUEST_MAX,
  );
  ...
  assert.deepEqual(
    [
      ...partitionFirst.acknowledged.record.items,
      ...partitionSecondAcknowledged.record.items,
    ]
  ...
  const [partitionFinalizeResult, partitionAbortResult] =
    await Promise.allSettled([partitionFinalize, partitionStaleAbort]);
  ...
  const [crossRowAbortResult, crossRowFinalizeResult] =
    await Promise.allSettled([crossRowAbortFirst, crossRowFinalizeSecond]);
...
test("upload ownership and execution leases survive takeover boundaries", async () => {
  ...
  leaseStore.setNowUnixMilliseconds(
    staleOwnerClaim.record.leaseExpiresAtUnixMilliseconds,
  );
  ...
  assert.equal(
    takeover!.record.ownerEpoch,
    staleOwnerClaim.record.ownerEpoch + 1,
  );
  await assert.rejects(
    () =>
      sealEncryptedWalletBackupUploadBatch({
        ...
        plannedBatch: plannedUploadBatchForTest(
          keyHandle,
          head,
          staleOwnerClaim.record.attemptId,
        ),
        store: leaseStore,
      }),
  ...
  assert.equal(
    restartedSingle!.record.attemptId,
    firstVaultAttempt.record.attemptId,
  );
  assert.equal(
    restartedSingle!.record.ownerEpoch,
    firstVaultAttempt.record.ownerEpoch + 1,
  );
  ...
  assert.equal(
    renewedSingle!.record.ownerEpoch,
    restartedSingle!.record.ownerEpoch + 1,
  );
  await assert.rejects(
    () =>
      sealEncryptedWalletBackupUploadBatch({
  ...
  const idempotentInput = {
    ...
    plannedBatch: plannedUploadBatchForTest(
      keyHandle,
      head,
      idempotentClaim.record.attemptId,
    ),
    store: idempotentStore,
  };
  const idempotentFirst =
    await sealEncryptedWalletBackupUploadBatch(idempotentInput);
  ...
  const executionBatch = await sealEncryptedWalletBackupUploadBatch({
    ...
    plannedBatch: plannedUploadBatchForTest(
      keyHandle,
      head,
      executionClaim.record.attemptId,
    ),
  ...
  delayedStore.setNowUnixMilliseconds(
    delayedClaim.record.leaseExpiresAtUnixMilliseconds,
  );
  ...
  assert.equal(
    delayedTakeover!.record.ownerEpoch,
    delayedClaim.record.ownerEpoch + 1,
  );
  releaseSigning();
...
test("linked CAS retry, crash rehydration, and DB-time exhaustion remain exact", async () => {
  ...
  assert.equal(
    secondRestart.record.retryNotBeforeUnixMilliseconds,
    secondBoundary,
  );
      ...
      for (let count = 0; count <= 3; count += 1) {
        for (const retryBoundary of [null, 1_700_000_005_000] as const) {
          for (const retryStreak of [0, 1] as const) {
            ...
            if (validShape && validStreak && validLifecycle)
...
test("rehydration rejects impossible execution history and quota-sized hostile heads", async () => {
  ...
  const validBoundary = rewriteUploadBatchTargetReferenceCounts(
    sealed.record,
    3,
    255,
  );
  ...
  const quotaOverflow = rewriteUploadBatchTargetReferenceCounts(
    sealed.record,
    3,
    256,
  );
...
test("persisted child parent authority is exact and rejects corruption", async () => {
  ...
  const childManifest = await prepareIncrementalEncryptedWalletBackupManifest({
    ...
    snapshotStore: acceptingSnapshotSealStore(),
    runtime: deterministicRuntime([
      new Uint8Array(16).fill(40),
      new Uint8Array(12).fill(41),
    ]),
  });
...
test("upload mutation adapters reject mutation, repeated, late, and wrong callbacks", async () => {
      ...
      sealEncryptedWalletBackupUploadAttempt({
        ...
        store: {
          ...
          async sealActiveUploadAttempt<T>(
            ...
            return repeatedBase.sealActiveUploadAttempt(
              candidate,
              lease,
              (record: never) => {
  ...
  assert.throws(
    () => lateCallback!(lateCandidate as never),
    /callback is invalid/,
  );
  ...
  const batchClaim = await uploadAttemptClaimForTest(
    keyHandle,
    head,
    batchBase,
    "47".repeat(16),
  );
  const plannedBatch = plannedUploadBatchForTest(
    keyHandle,
    head,
  ...
  const faults: Array<Readonly<{ name: string; run: () => Promise<void> }>> = [
    {
      ...
      async run() {
        ...
        fixture.store.setNowUnixMilliseconds(
          fixture.claim.record.leaseExpiresAtUnixMilliseconds,
        );
        await assert.rejects(
          () =>
            claimEncryptedWalletBackupUploadAttempt({
              ...
              store: {
                ...
                async claimActiveUploadAttempt<T>(
                  ...
                  return fixture.store.claimActiveUploadAttempt(
                    query,
                    (record: never) => {
        ...
        await assert.rejects(
          () =>
            uploadEncryptedWalletBackupBatch({
              ...
              clock: { nowUnixSeconds: () => 1_700_000_000 },
              objectUrl: (objectId) =>
                `https://backup.example.test/${objectId}`,
              remote: {
                async putObject() {
                  remoteCalls += 1;
                  return { status: "stored" as const };
...
test("manifest preparation rejects rows from different committed wallet revisions", async () => {
  ...
  const first = await prepareEncryptedWalletBackupProof(
    proofInputAtCounter(keyHandle, 0, 1),
  );
  const second = await prepareEncryptedWalletBackupProof(
...
test("manifest authority requires final transactional proof-set revalidation", async () => {
  ...
  const object = await prepareEncryptedWalletBackupObject({
    ...
    generation: 1,
    runtime: deterministicRuntime([
      new Uint8Array(16).fill(62),
      new Uint8Array(12).fill(63),
    ]),
  });
  for (const mutate of [
  ...
  const concurrent = await Promise.allSettled(
    [0, 1].map(() =>
      authenticateEncryptedWalletBackupRequest({
        ...
        replayStore: {
          async consumeReplayNonce(input: { replayNonce: string }) {
            if (concurrentSeen.has(input.replayNonce))
...
test("realm separation and exact-object capability provenance fail closed", async () => {
  ...
  assert.equal(
    (await createEncryptedWalletBackupKeyHandle({ seed: SEED, realm: valid64 }))
      .realm,
    valid64,
  );
});
...
test("request-auth scalar derivation rejects zero and out-of-range candidates and caps exhaustion", async () => {
  ...
  const overOrder = fromHex(
    "ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff",
  );
  const validOne = fromHex(`${"00".repeat(31)}01`);
  const accepted = scalarCandidateRuntime([
    new Uint8Array(32),
    order,
    overOrder,
    validOne,
  ...
  const exhausted = scalarCandidateRuntime(
...
test("preparation validates seed, counter, classifier facts, proof class, fields, and keyset wire", async () => {
  ...
  for (const override of [
    ...
    await assert.rejects(
      () =>
        prepareEncryptedWalletBackupProof(
          withProofStore(proofInput(keyHandle), null, override),
        ),
  ...
  assert.throws(
    ...
    /Unrecognized|mixes Base64 alphabets/,
  );
});
...
test("authoritative snapshot binds proof bytes and every validated conditional-keyset field", async () => {
  ...
  for (const metadataOverride of [
    ...
    // A later claimed expiry must not extend the cryptographically verified keyset lifetime.
    {
      finalExpiryUnixSeconds:
        ctfInput.ctfMetadata.finalExpiryUnixSeconds + 10_000,
    },
  ]) {
    const spoofed = {
      ...ctfInput,
  ...
  await assert.rejects(
    () =>
      prepareEncryptedWalletBackupProof(
        withProofStore(ctfInput, foreignMintEvidence),
      ),
    /validated conditional keyset/,
  );
  for (const changed of [
...
test("proof-store transaction is the only synchronous exact-row authority boundary", async () => {
  assert.equal(
    "prepareEncryptedWalletBackupStorageSnapshot" in BackupModule,
    false,
  );
  const keyHandle = await createEncryptedWalletBackupKeyHandle({
    seed: SEED,
  ...
  await assert.rejects(
    () =>
      prepareEncryptedWalletBackupProof(
        withProofStore(valid, null, { proofId: "ff".repeat(32) }),
      ),
...
test("proof field, curve, amount, time, and keyset boundaries fail closed", async () => {
  ...
  for (const input of invalidInputs) {
    await assert.rejects(() =>
      prepareEncryptedWalletBackupProof(input as never),
    );
  }
  const ctf = {
    ...base,
  ...
  await assert.rejects(
    () => prepareEncryptedWalletBackupProof(ctf),
    /outcome label is invalid/,
  );
  await assert.rejects(
    () =>
      prepareEncryptedWalletBackupProof({
...
test("packing rejects duplicates, count and canonical-size overflow, and collision exhaustion", async () => {
  ...
  const runtime = deterministicRuntime(
    new Array(8).fill(fromHex(vector.inputs.objectIdHex)),
  );
  await assert.rejects(
    () =>
      prepareEncryptedWalletBackupObject({
...
test("decrypt rejects metadata, body, AAD, tamper, truncation, padding, and noncanonical CBOR generically", async () => {
  ...
  const nonzeroPadding = await encryptRawFrame(
    fromHex(vector.expected.canonicalCborHex),
    1,
  );
  await assert.rejects(
  ...
  for (const mutation of [
    ...
    mutation(changed);
    const object = await encryptRawFrame(
      encode([1, 1, [changed]], rfc8949EncodeOptions),
      0,
    );
    await assert.rejects(
      () =>
...
function withProofStore(
  ...
  const identityKeyset =
    keysetKind === 0 ? `legacy:${toHex(fromBase64(keysetId))}` : keysetId;
  ...
  const dleq =
    input.proof.dleq === undefined
      ? null
      : [
          fromHex(input.proof.dleq.e),
          fromHex(input.proof.dleq.s),
          fromHex(input.proof.dleq.r),
        ];
...
async function acknowledgeTargetUploadsForTest(input: {
  ...
  const claim =
    input.claim ??
    (await uploadAttemptClaimForTest(
      input.keyHandle,
      input.targetHead,
      store,
      input.attemptId,
...
function rewriteUploadBatchTargetReferenceCounts(
  ...
  const ids = new Set(
    [...pageReferences, ...chunkReferences].map((reference) =>
      toHex(reference[0] as Uint8Array),
    ),
  );
  const digests = new Set(
    [...pageReferences, ...chunkReferences].map((reference) =>
      toHex(reference[1] as Uint8Array),
    ),
  );
...
function indexedReferenceBytes(
  domain: number,
  index: number,
  length: number,
): Uint8Array {
...
async function createMultiBatchTargetForTest(
  ...
  const proofs = await Promise.all(
    Array.from({ length: 17 }, (_, index) =>
      prepareEncryptedWalletBackupProof(
        proofInputAtCounter(keyHandle, 100 + index),
      ),
    ),
  );
  ...
  const manifest = await prepareEncryptedWalletBackupManifest({
    ...
    snapshotStore: acceptingSnapshotSealStore(),
    runtime: deterministicRuntime([
      new Uint8Array(16).fill(191),
      new Uint8Array(12).fill(192),
    ]),
  });
  return {
  ...
  const partition = (attemptId: unknown, targetManifestDigest: unknown) =>
    [...batches.values()].filter(
      (record) =>
        ...
        record.targetManifestDigest === targetManifestDigest,
    );
  return {
    ...
    async claimActiveUploadAttempt<T>(
      ...
      record.leaseExpiresAtUnixMilliseconds =
        databaseNow + Number(query.leaseDurationMilliseconds);
      try {
    ...
    async sealUploadBatch<T>(
      ...
      if (existing !== undefined) {
        if (!isDeepStrictEqual(existing, batch)) {
          throw new Error(
            "backup upload batch id conflicts with different content",
          );
        }
        return seal({
      ...
      if (
        partition(batch.attemptId, batch.targetManifestDigest).some((record) =>
          ["abort-uncertain", "finalized", "abandoned"].includes(
            String(record.state),
          ),
        )
      ) {
      ...
      const existingItems = partition(
        batch.attemptId,
        batch.targetManifestDigest,
      ).flatMap(
        (record) =>
          record.items as Array<{ objectId: string; objectDigest: string }>,
      );
        ...
        if (
          existingItems.some(
            (value) =>
              ...
              value.objectDigest === item.objectDigest,
          )
        )
    ...
    async validateUploadBatchExecution<T>(
      ...
      assertExactPersistedRecord(
        current,
        batch,
        "backup upload execution validation",
      );
      if (
        current.executionLeaseExpiresAtUnixMilliseconds === null ||
        databaseNow >= Number(current.executionLeaseExpiresAtUnixMilliseconds)
    ...
    async transitionUploadBatch<T>(
      ...
      validateClaim(
        claimRecord,
        next.state === "abandoned" ? ["abort-uncertain"] : ["active"],
      );
      ...
      const beforeAttempt = structuredClone(
        attempts.get(String(claimRecord.attemptId)),
      );
    ...
    async fenceUploadAttemptForAbort<T>(
      ...
      attempts.get(String(claimRecord.attemptId))!.lifecycle =
        "abort-uncertain";
        ...
        return async (
          ...
          if (info.includes("request-auth-scalar")) {
            ...
            if (candidate === undefined)
              throw new Error("missing scalar candidate");
