...
test('authorization decoder accepts exactly one canonical BackupV1 value', async () => {
  ...
  const canonicalProof = decodeEncryptedWalletBackupAuthorizationHeader(fixture.headerValues)
  ...
  assert.equal(ENCRYPTED_WALLET_BACKUP_REQUEST_PROOF_MAX_BYTES, 4_096)
  assert.equal(ENCRYPTED_WALLET_BACKUP_AUTHORIZATION_HEADER_MAX_CHARACTERS, 5_471)
})
...
test('authorization proof limit accepts exact max and rejects max plus one', () => {
  const exact = new Uint8Array(ENCRYPTED_WALLET_BACKUP_REQUEST_PROOF_MAX_BYTES)
  const over = new Uint8Array(ENCRYPTED_WALLET_BACKUP_REQUEST_PROOF_MAX_BYTES + 1)
  assert.equal(
    `BackupV1 ${base64Url(exact)}`.length,
    ENCRYPTED_WALLET_BACKUP_AUTHORIZATION_HEADER_MAX_CHARACTERS,
  )
  assert.equal(
    decodeEncryptedWalletBackupAuthorizationHeader([`BackupV1 ${base64Url(exact)}`]).byteLength,
    ENCRYPTED_WALLET_BACKUP_REQUEST_PROOF_MAX_BYTES,
  )
  assert.throws(
    () => decodeEncryptedWalletBackupAuthorizationHeader([`BackupV1 ${base64Url(over)}`]),
    /authorization header is invalid/,
  )
})
...
test('delegated authentication binds configured origin, raw route, method, payload, and enrollment', async () => {
  ...
  const result = await authenticateAndDecodeEncryptedWalletBackupDelegatedServerRequest({
...
test('route, origin, URL, method, and payload confusion fail closed', async () => {
  const cases: readonly Readonly<{
    mutate: (fixture: Awaited<ReturnType<typeof delegatedFixture>>) => Record<string, unknown>
  }>[] = [
    {
      mutate: (fixture) => ({
        rawTarget: `${fixture.rawTarget}/other`,
...
test('proof self-verification fails before active enrollment is inspected', async () => {
  ...
  const proofTuple = decode(decodeEncryptedWalletBackupAuthorizationHeader(fixture.headerValues))
  assert.ok(Array.isArray(proofTuple))
  proofTuple[13] = new Uint8Array(64)
  const invalidHeader = `BackupV1 ${base64Url(encodeCanonicalBackupCbor(proofTuple))}`
...
test('verified proof replay authority cannot be cloned or reconstructed', async () => {
  ...
  const authenticated = await consumeEncryptedWalletBackupVerifiedRequestReplay({
...
test('replay-store outages remain typed unavailable failures', async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) => error instanceof EncryptedWalletBackupReplayStoreUnavailableError,
  )
})
...
test('operation request maxima reject max plus one without unbounded decoding', () => {
  ...
  assert.equal(encryptedWalletBackupDelegatedPayloadMaximumBytes('head-get'), 0)
  ...
  for (const current of cases) {
    assert.throws(() => current.decode(new Uint8Array(current.maximum)), /request is invalid/)
    assert.throws(() => current.decode(new Uint8Array(current.maximum + 1)), /request is invalid/)
  }
})
...
async function delegatedFixture(input: {
  ...
  const route = routeFor(input.operation, keyHandle.vaultId, input.objectId ?? OBJECT_ID)
  const rawTarget = pathFor(route)
...
function deterministicRuntime() {
  ...
  return {
    ...
    getRandomValues<T extends ArrayBufferView | null>(array: T): T {
      ...
      new Uint8Array(array.buffer, array.byteOffset, array.byteLength).fill(counter)
      counter += 1
      return array
    },
  }
...
function headCasPayload(input: {
  ...
  const referenceDigest =
    input.referenceDigest === undefined ? sha256(references) : hexToBytes(input.referenceDigest)
  const parent =
    input.generation === 1
      ? null
      : [
          input.generation - 1,
          hexToBytes(input.headParentDigest ?? input.expectedManifestDigest ?? '00'.repeat(32)),
        ]
  ...
  return {
    payload: encodeCanonicalBackupCbor([
      ...
      'head-cas',
      hexToBytes(ATTEMPT_ID),
      input.expectedManifestDigest === null ? null : hexToBytes(input.expectedManifestDigest),
      head,
      references,
    ]),
  }
...
function mutateTuple(payload: Uint8Array, index: number, value: unknown): Uint8Array {
