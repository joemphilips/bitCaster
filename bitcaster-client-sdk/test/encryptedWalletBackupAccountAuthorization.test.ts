...
const ACCOUNT_URL =
  "https://backup.example/v1/encrypted-wallet-backup/realms/test/vaults:enroll";
...
test("account authorization binds the exact canonical intent", async () => {
  ...
  const decoded = decodeEncryptedWalletBackupAccountRequest(
    operation.canonicalRequest,
  );
  const vector = await readVector();
  assert.equal(
    bytesToHex(decoded.canonicalIntent),
    vector.expected.canonicalIntentHex,
  );
  assert.equal(decoded.intentDigest, vector.expected.intentDigest);
  assert.equal(
    new TextDecoder().decode(decoded.authorization),
    vector.expected.authorizationJson,
  );
  assert.equal(
...
test("account request decoder rejects an intent digest substitution", async () => {
  ...
  assert.throws(
    () => decodeEncryptedWalletBackupAccountRequest(request),
    /account request/,
  );
});
...
test("account intent rejects an invalid delegated public key", async () => {
  ...
  assert.throws(
    () =>
      decodeEncryptedWalletBackupAccountIntent(
        encodeCanonicalBackupCbor(tuple),
      ),
    /public key/,
  );
});
...
test("authorization scheme uses one strict client/server grammar", () => {
  ...
  for (const invalid of ["1scheme", "scheme-", "scheme.", "UPPER", ""]) {
    assert.throws(
      () => requireEncryptedWalletBackupAuthorizationScheme(invalid),
      /scheme/,
    );
  }
});
...
test("account authorization rejects corrupted signed-event fields", async () => {
  ...
  for (const mutate of [
    ...
    const authorization = mutateCanonicalAuthorization(
...
test("account authorization rejects future events and reordered tags", async () => {
  ...
  const future = mutateAndResignAuthorization(
    decoded.authorization,
    (event) => {
      event.created_at = NOW + 61;
    },
  );
  const reordered = mutateAndResignAuthorization(
    decoded.authorization,
    (event) => {
...
test("account request rejects hostile nested CBOR before semantic decoding", () => {
  ...
  assert.throws(
    () => decodeEncryptedWalletBackupAccountRequest(hostile),
    /cbor|account request/,
  );
});

async function prepareOperation(input?: {
  action?: "enroll" | "revoke" | "delete";
}) {
...
function mutateCanonicalAuthorization(
  ...
  const event = JSON.parse(new TextDecoder().decode(authorization)) as Record<
    string,
    unknown
  >;
  mutate(event);
...
function mutateAndResignAuthorization(
  ...
  const event = JSON.parse(
    new TextDecoder().decode(authorization),
  ) as MutableSignedEvent;
  mutate(event);
  resignEvent(event);
  return new TextEncoder().encode(JSON.stringify(event));
}
...
function resignEvent(event: MutableSignedEvent): void {
  ...
  event.sig = bytesToHex(
    schnorr.sign(hexToBytes(id), PRIVATE_KEY, new Uint8Array(32)),
  );
}
...
async function readVector(): Promise<{
  ...
  return JSON.parse(
    await readFile(
      new URL(
        "../../test-vectors/encrypted-wallet-backup-account-auth-v1.json",
        import.meta.url,
      ),
      "utf8",
    ),
