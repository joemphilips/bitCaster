...
test('conditional swap previews fail before mint when outputs change keyset', () => {
  ...
  const preview: ConditionalSwapPreview = {
    ...
    inputs: [lockedProof(swapContext('trade-output-keyset'), conditionalKeysetId, 100)],
    outputDataByLabel: {
...
class MemoryProofOperationStore implements ProofOperationStore {
  ...
  async getProofOperation(operationId: string): Promise<ProofOperationRecord | null> {
    return this.records.get(operationId) ?? null
  }

  async prepareProofOperation(input: PrepareProofOperationInput): Promise<ProofOperationRecord> {
